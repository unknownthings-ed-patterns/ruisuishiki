...
export const ALGEBRA_NECESSARY_SUFFICIENT_SERIES: LearnerSeries = {
  ...
  derivation: `**「十分」と「必要」を集合で見る**

「$A$ ならば $B$」が真のとき、$A$ は $B$ の **十分条件**、$B$ は $A$ の **必要条件** といいます。これは集合で考えると一目でわかります。

**集合の包含で考える**

「$A$ ならば $B$」が真ということは、$A$ が成り立つすべての場合、$B$ も成り立つ——つまり「$A$ を満たすもの全体」が「$B$ を満たすもの全体」に **含まれる**：

$$A \\subset B$$

「$A$ は $B$ より小さい集合」と理解できます。
...
$\\{4, 8, 12, 16, \\ldots\\} \\subset \\{2, 4, 6, 8, \\ldots\\}$ なので、$A \\subset B$。「4 の倍数なら 2 の倍数」は真です。

**「十分」と「必要」の意味**

- **$A$ は $B$ の十分条件**：$A$ さえあれば、$B$ は必ず成り立つ。「$A$ で**十分**」
- **$B$ は $A$ の必要条件**：$A$ が成り立つには、$B$ が成り立っていることが**不可欠**

「4 の倍数（$A$）」と「2 の倍数（$B$）」では：

- $A$ は $B$ の十分条件：「4 の倍数なら 2 の倍数」（4 の倍数で十分）
- $B$ は $A$ の必要条件：「4 の倍数になるには、まず 2 の倍数でなければならない」

**4つのパターン**

「$A$ ならば $B$」と「$B$ ならば $A$」の真偽の組み合わせで、4 つのパターンがあります：

| $A \\Rightarrow B$ | $B \\Rightarrow A$ | $A$ は $B$ の何条件？ |
|---|---|---|
| 真 | 偽 | **十分条件**（だけ） |
| 偽 | 真 | **必要条件**（だけ） |
| 真 | 真 | **必要十分条件** |
...
**必要十分条件 = 「$\\Leftrightarrow$」**

$A \\Rightarrow B$ と $B \\Rightarrow A$ が **両方とも真** のとき、$A$ と $B$ は **同じ集合**：

$$A = B$$

これを **「$A$ は $B$ と同値」「$A$ は $B$ の必要十分条件」** と言います。記号で $A \\Leftrightarrow B$。

例：「$x^2 = 9$」と「$x = \\pm 3$」は完全に同値。両方とも同じ解集合 $\\{3, -3\\}$ を表します。

**反例を1つ見つければ「ならば」は壊れる**

「$A \\Rightarrow B$」が真かどうかを判定するには、「**$A$ を満たすが $B$ を満たさない例**」が1つでもあれば、その時点で偽です。

例：「$x$ は素数 $\\Rightarrow$ $x$ は奇数」は偽。$x = 2$ は素数だが奇数でないので。

**論理の世界の中心**

「ならば」「同値」「必要・十分」——これらは数学の証明の基本道具。すべての定理が「$A$ ならば $B$」の形で書かれ、それぞれの真偽を集合の包含で確かめます。

「**ならば」は、左の集合が右の集合に含まれることに他ならない**——これが論理学の出発点です。`,
  steps: [
    {
      id: "step1",
      position: 1,
...
export const ALGEBRA_PERMUTATION_SERIES: LearnerSeries = {
  ...
  derivation: `**順列 P(n, 2) は「順番つきで 2 つ並べる方法」**

$n$ 個から 2 つを **順番つきで** 並べる場合の数：

$$P(n, 2) = n \\times (n - 1)$$

「**$n$ × $(n-1)$**」——これは **2 つのステップ** から自然に出てきます。

**ステップごとに考える**

1 番目に並べる人を選ぶ：$n$ 通り
2 番目に並べる人を選ぶ：残りから $n - 1$ 通り

両方を **同時に** 選ぶ場合の数は、それぞれの選び方を **掛け算**：

$$n \\times (n - 1)$$

**たとえば 3 人 A, B, C から 2 人を順に並べる**

- A を 1 番目 → 2 番目は B か C：(A,B), (A,C)
- B を 1 番目 → 2 番目は A か C：(B,A), (B,C)
- C を 1 番目 → 2 番目は A か B：(C,A), (C,B)

合計 6 通り。公式で計算：$3 \\times 2 = 6$。

**組合せ C(n, 2) との違い**

ここで重要な違い：順列では **(A, B) と (B, A) は別物** として数えます（順番が違うから）。

組合せ $C(n, 2)$ では同じ組として数えるので、その半分の $n(n-1)/2$。
...
「**$n$ から始めて、1 ずつ減らしながら $r$ 個掛ける**」。階乗を使うと：

$$P(n, r) = \\frac{n!}{(n - r)!}$$

**身近な順列**

- 5 人でリレーの **第 1 走者・第 2 走者** を決める：$P(5, 2) = 20$ 通り
- 10 個の数字から 4 桁の暗証番号（同じ数字使えない）：$P(10, 4) = 5040$ 通り
- アルファベット 26 文字から 3 文字を並べる：$P(26, 3) = 15600$ 通り

「**順番を区別する**」状況では、すべて順列です。
...
問題文を読んで、「**順番が大事か？**」を判定するのが鍵。順番が大事なら順列、組合せだけなら組合せ——これが「場合の数」の基本姿勢です。`,
  steps: [
    {
      id: "step1", position: 1,
...
export const ALGEBRA_DIVISOR_COUNT_SERIES: LearnerSeries = {
  ...
  derivation: `**約数の個数は素因数分解から計算できる**

整数 $n$ を素因数分解した形が $n = p^a \\times q^b$ なら、約数の個数は：

$$(a + 1)(b + 1)$$

3 つ以上の素因数の場合も同じ要領で：

$$n = p^a \\times q^b \\times r^c\\quad\\Longrightarrow\\quad \\text{約数の個数} = (a+1)(b+1)(c+1)$$

「**指数に 1 を足して、全部掛ける**」——なぜこんなに簡単に計算できるのでしょうか。

**約数は「素因数の選び方」**

$n = 12$ で確かめます。$12 = 2^2 \\times 3$。約数を全部書き出すと：

$$1, 2, 3, 4, 6, 12$$

6 個あります。これは $(2 + 1)(1 + 1) = 6$ と一致。

なぜ 6 個か？ 各約数は次の形：

$$2^x \\times 3^y$$

- $x$ は $0, 1, 2$ の **3 通り**（$2^0, 2^1, 2^2$）
- $y$ は $0, 1$ の **2 通り**（$3^0, 3^1$）

それぞれ独立に選べるので、組合せは $3 \\times 2 = 6$ 通り——これがそのまま約数の個数です。

**$(a + 1)$ の「+1」の意味**

各素因数の指数が $a$ のとき、選べる **指数の値** は $0, 1, 2, \\ldots, a$ の **$(a + 1)$ 通り**。「0 を含むから 1 を足す」というのが「$+1$」の理由。

**たとえば**

| $n$ | 素因数分解 | 計算 | 約数の個数 |
|---|---|---|---|
| 12 | $2^2 \\cdot 3$ | $(2+1)(1+1)$ | 6 |
| 36 | $2^2 \\cdot 3^2$ | $(2+1)(2+1)$ | 9 |
| 60 | $2^2 \\cdot 3 \\cdot 5$ | $(2+1)(1+1)(1+1)$ | 12 |
| 100 | $2^2 \\cdot 5^2$ | $(2+1)(2+1)$ | 9 |
| 1000 | $2^3 \\cdot 5^3$ | $(3+1)(3+1)$ | 16 |

**応用：完全数・平方数の特徴**

- **平方数**（$1, 4, 9, 16, 25, \\ldots$）の約数の個数は **必ず奇数**。なぜなら、$n = p^{2a}$ などの形で、$(2a+1)$ は奇数だから。
- **素数** の約数の個数はちょうど **2**（自分と 1 のみ）。

**約数の和も計算できる**

実は、約数の **和** にも公式があります：

$$\\sigma(n) = \\frac{p^{a+1} - 1}{p - 1} \\cdot \\frac{q^{b+1} - 1}{q - 1}$$

たとえば $12 = 2^2 \\cdot 3$ なら：

$$\\sigma(12) = \\frac{2^3 - 1}{1} \\cdot \\frac{3^2 - 1}{2} = 7 \\cdot 4 = 28$$

確かめると $1 + 2 + 3 + 4 + 6 + 12 = 28$ で一致。

**整数の世界の地図**

素因数分解は、整数を「**素数のレゴブロック**」に分解する作業。一度分解すれば、
...
export const ALGEBRA_VARIANCE_SERIES: LearnerSeries = {
  ...
  derivation: `**分散は「散らばり具合」を測る**

平均 $\\bar{x}$ がデータの **真ん中** を表すなら、**分散** はデータが **平均からどれくらい散らばっているか** を表します。

$$\\text{分散} = \\frac{(x_1 - \\bar{x})^2 + (x_2 - \\bar{x})^2 + \\cdots + (x_n - \\bar{x})^2}{n}$$

「**各データと平均の差**（偏差）**を 2 乗して、平均する**」——これが分散です。

**なぜ「2 乗」するのか**

「偏差そのまま」を平均すると、必ず **0** になってしまいます。なぜなら、平均より大きい値も小さい値もあって、プラスとマイナスがちょうど打ち消し合うからです。

たとえば $1, 2, 3, 4, 5$ の平均は $3$。偏差は $-2, -1, 0, 1, 2$。合計は $0$。

これでは「散らばり具合」を表せません。

そこで、**2 乗して全部プラスにしてから平均する**。これなら散らばりの大きさが反映されます。
...
データ：$3, 3, 3, 3, 3$（全部同じ）の場合、偏差はすべて $0$、分散も $0$。**散らばりがない** ことを意味します。

データ：$0, 0, 3, 6, 6$ の場合、平均は $3$、偏差は $-3, -3, 0, 3, 3$、2 乗は $9, 9, 0, 9, 9$、和は $36$、分散は $36/5 = 7.2$。先ほどの 2 より大きい——これは **散らばりが大きい** ことを反映しています。

**標準偏差は「分散のルート」**

分散は値を 2 乗してしまっているので、単位がデータと違います（円なら円²）。そこで、**ルート** を取ったものを **標準偏差** といいます：

$$\\sigma = \\sqrt{\\text{分散}}$$

標準偏差は、データと同じ単位で「平均からの典型的なずれ」を表します。

**「データの中心 + 散らばり」で分布を表現**

統計でデータを要約するとき、最も基本的な 2 つの指標が：

- **中心**：平均、中央値
- **散らばり**：分散、標準偏差

これら 2 つで、データの「**だいたいの形**」が伝わります。

**応用：偏差値**

学校のテストの偏差値は、標準偏差を使って計算されます：

$$\\text{偏差値} = 50 + 10 \\times \\frac{得点 - 平均}{\\text{標準偏差}}$$

「平均より標準偏差 1 個分上 → 偏差値 60」「2 個分上 → 偏差値 70」。

これにより、**異なるテストでも自分の位置が比較できる** ようになります。

**統計学の出発点**

分散と標準偏差は、統計学のあらゆる分野で登場する **最も基本的な道具**。
...
「**平均からのずれを 2 乗して平均する**」というシンプルなアイデアが、データを語る上での基本言語になっています。`,
  steps: [
    {
      id: "step1", position: 1,
