...
3辺を $a, b, c$（$c$ が斜辺）とすると：

$$a^2 + b^2 = c^2$$

「**2 つの辺の 2乗を足すと、斜辺の 2乗になる**」。ぴったり噛み合うのは、直角があるおかげ。

**なぜ成り立つのか——「長さの話」を「面積の話」にする**

長さどうしの関係を、長さのまま証明しようとしても堂々巡り。そこで、長さを **面積に翻訳** してみます。これがこの証明の **核となる考え方** です。

1辺が $(a + b)$ の大きな正方形を 1 つ用意して、**同じ正方形を「2 通り」に分けて見てみる**——これが証明の心臓部分です。

**数え方その 1**：a 側と b 側でまっすぐ切る

<<EXPANSION_SQUARE>>

縦も横も「a で 1 回、b で 1 回」切ると、$4$ つの部分に分かれます。$a^2$ の正方形、$b^2$ の正方形、$ab$ の長方形が $2$ つ：

$$(a + b)^2 = a^2 + ab + ab + b^2 = a^2 + 2ab + b^2$$

**数え方その 2**：同じ大きさの正方形に、4 枚の直角三角形を斜めに敷きつめる

<<PYTHAGORAS_PROOF>>

すると真ん中にぴったり **1 辺が $c$ の正方形** が現れます。だから：

$$\\text{大きな正方形の面積} = \\underbrace{4 \\times \\dfrac{ab}{2}}_{4 枚の直角三角形} + \\underbrace{c^2}_{中の正方形} = 2ab + c^2$$

**両者は同じ大きさの正方形を表しているから**：
...
**この証明の「考え方」を改めて**：長さを長さのまま比べようとしても進まないとき、**面積に翻訳して「同じものを 2通りで数える」**——これは数学の他の場所でも何度も再登場する、強力な道具です。釣り方そのもの。
...
この系列の Step 1〜4 では、これをひたすら繰り返しました。

**4 問で同じ操作をくり返す——そして何が起きたか**

- Step 1：$(3, 4, 5)$　$9 + 16 = 25$ ✓
- Step 2：$(6, 8, 10)$　$36 + 64 = 100$ ✓
- Step 3：$(5, 12, 13)$　$25 + 144 = 169$ ✓
- Step 4：$(8, 15, 17)$　$64 + 225 = 289$ ✓

**同じ計算を 4 回繰り返したのに、毎回ぴったり整数の斜辺が出てきた**——これ、奇跡みたいなことです。ふつうの 2辺なら、$\\sqrt{a^2 + b^2}$ はだいたい無理数（小数で割り切れない数）になるはずなのに、上の 4 組ではぴったり整数。

**整数組（ピタゴラス数）の発見**

$a^2 + b^2 = c^2$ が 3 つとも整数で成り立つ組を [ピタゴラス数] と呼びます。出会いやすい代表たち：

- $(3, 4, 5)$、$(5, 12, 13)$、$(8, 15, 17)$、$(7, 24, 25)$ …

これらは偶然できているわけではなく、$(m^2 - n^2,\\ 2mn,\\ m^2 + n^2)$ という公式から **無限に作り出せる** ことが知られています。たとえば $m = 2, n = 1$ なら $(3, 4, 5)$。[互いに素] な $m, n$ を選ぶといちばん基本的な組（**原始ピタゴラス数**）が出てきます。

**Step 5：質を変えてみる——正方形の対角線**

Step 1〜4 が「同じ操作を繰り返す」だったとして、Step 5 では一段、**問いの質を変えます**。「**直角を挟む 2辺が等しい**（$a = b$）」場合はどうなるか？
...
**正方形の対角線は、辺の必ず $\\sqrt{2}$ 倍になる**——整数の世界（ピタゴラス数）から、無理数（ルートを含む数）の世界へ。同じ式が別の景色を見せてくれます。

これが Step 1〜4 と Step 5 の違い：「**同じ操作を繰り返す**」から「**操作の中身を一段変える**」へ——変化の質が変わるところに、新しい発見があります。

────────

**もっと深く** — どこで使われる、どこにつながる

**距離の計算**：座標平面で 2 点 $(x_1, y_1)$、$(x_2, y_2)$ の距離は

$$\\sqrt{(x_2 - x_1)^2 + (y_2 - y_1)^2}$$

——これはピタゴラスの定理そのもの。直角がたくさん集まっている限り、ピタゴラスは効きます。3 次元の空間でも同じく、$\\sqrt{(\\Delta x)^2 + (\\Delta y)^2 + (\\Delta z)^2}$。

**別の見方への橋**：直角三角形を「3辺の長さの関係」で見るとピタゴラス、「角と辺の比」で見ると [タンジェント] や三角比。同じ三角形に対する、2 つの違う問いかけです。

**ピタゴラスの裏返し——フェルマーの最終定理**：$a^n + b^n = c^n$ で $n \\geq 3$ では整数解がない（$n = 2$ だけが特別）——超有名なこの定理は、ピタゴラスの「$n = 2$ なら無限にある」を裏返した問い。$n$ をひとつ動かすだけで、世界の景色がガラリと変わります。`,
  steps: [
    {
      id: "step1",
      position: 1,
