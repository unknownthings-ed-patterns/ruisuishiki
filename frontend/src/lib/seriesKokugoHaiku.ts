  ...
  steps: [
    ...
    // ── Step2 exercise 同：音数の識別（拗音・促音・長音・撥音） ──────────
    {
      ...
      hints: [
        ...
        {
          layer: 3,
          text: "「きゅ／う／しょ／く／で」で $5$、「ラ／ー／メ／ン／た／べ／た」と数えると中は $7$、「お／い／し／い／っ」で $5$。合わせて $17$。小さい字は前の音に乗るだけ、のばす・つまる・はねるは一回ずつ——これが数え方のきまり。",
        },
      ],
    ...
    // ── Step5 creation 同：本歌取・一語差し替え ──────────
    {
      ...
      hints: [
        ...
        {
          layer: 3,
          text: "芭蕉は「かわず（蛙）」が池に飛びこむ音をとらえたね。「ふるいけや」「とびこむ」「みずのおと」はそのまま。あなたは同じ場所に、$3$ 音の「池に飛びこむ何か」を置く番。たとえば——「古池や芭蕉飛びこむ水の音」（飛びこんだのは、作者本人！）。「古池やアヒル飛びこむ水の音」。かえるの席に、だれをすわらせるかで、句がまるごと変わるね。さあ、あなたは、だれを飛びこませる？",
        },
      ],
      variationFromPrevious: "same",
      compareWithStepId: "step1",
    ...
    // ── Step7 comparison 質的変化：定型と自由律の読み比べ ──────────
    {
      ...
      hints: [
        ...
        {
          layer: 3,
          text: "「せきをしても／ひとり」は数えると $9$ 音、器に入っていない。「わけいっても　わけいっても　あおいやま」は $6$・$6$・$5$——ぜんぶで $17$ 音あるけれど、五・七・五には分かれていない。器は、音の数だけでなく、分かれ方も作っているんだね。このように五・七・五の器を外して作る俳句を [自由律] という。それでもさびしさや、山の深さが伝わってくる——器があることの意味は、外した句とくらべると、よけいにはっきりするね。",
        },
      ],
