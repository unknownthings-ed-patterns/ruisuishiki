...
**$\\log_b v$ は「底 $b$ を何回かけたら $v$ になるか」の回数**。これが対数の正体です。

**指数と対数は同じ事実の 2 方向の書き方**
...
「$b^x = v$」と「$\\log_b v = x$」は **完全に同じ関係**。書き方が違うだけ。

**Step 1〜4：同じリズムを 4 回**

最初の 4 問は「**底を何回かけたら真数になるか**」を繰り返しました。$\\log_2 4, \\log_2 8, \\log_3 9, \\log_5 125$——底と真数が変わっても、考え方は同じ。「何回かければいいか」を探す。

**Step 5：質的変化——大きい数で**

Step 5 は $\\log_2 16$。$2$ を **$4$ 回** かけて $16$。Step 1 と Step 2 の延長線上にある問題ですが、真数が大きくなった分、$2$ の累乗（$2, 4, 8, 16, 32, \\dots$）を **頭の中で並べて数える** 感覚が育ちます。

「$2$ の累乗を $1, 2, 4, 8, 16$ と暗算で並べられる」——この $1$ つの技能が、対数の基礎体力です。

────────

**もっと深く** — なぜ対数がこんなに大事なのか

**桁数で語る道具**：指数は急激に大きくなります。$2^{30}$ は約 $10$ 億、$2^{40}$ は約 $1$ 兆。こんな数を「ふつうに」扱うのは大変ですが、**対数で表せば $30$ や $40$ という小さな数** に化けます。

「**何倍違うか**」を扱うとき、対数はいちばん自然な道具です。

**$3$ つの基本性質——「かけ算が足し算に変わる」魔法**

| 性質 | 内容 | なぜ？ |
|---|---|---|
| 1. かけ算 → 足し算 | $\\log_b (xy) = \\log_b x + \\log_b y$ | $b^p \\cdot b^q = b^{p+q}$ の逆 |
| 2. 割り算 → 引き算 | $\\log_b (x/y) = \\log_b x - \\log_b y$ | $b^p / b^q = b^{p-q}$ の逆 |
| 3. 累乗 → かけ算 | $\\log_b (x^n) = n \\log_b x$ | $(b^p)^n = b^{pn}$ の逆 |

これらは、指数法則を **逆向きに翻訳した** もの。

たとえば $100! = 1 \\cdot 2 \\cdots 100$ のような巨大な数も、対数で扱えば $\\log 100! = \\log 1 + \\log 2 + \\dots + \\log 100$ と **足し算の積み重ね** に変わります。

**現代社会で対数が顔を出す場面**

- **音の大きさ（デシベル dB）**：対数スケール（音圧 $10$ 倍で $+20$ dB）
- **地震のマグニチュード**：M が $1$ 違うと、エネルギーは約 $32$ 倍
- **pH（酸性・アルカリ性）**：水素イオン濃度の対数（pH が $1$ 違うと濃度 $10$ 倍）
- **コンピュータの計算量**：二分探索は $\\log_2 n$ 回で答えに辿り着く
- **音楽の音階**：$1$ オクターブで周波数 $2$ 倍 → 半音は $2^{1/12}$ 倍

これらすべて「**何倍違うか**」が本質の問題——だから対数が出てくる。

**特別な底——自然対数と常用対数**

- **底 $e \\approx 2.718$（ネイピア数）**：自然対数 $\\ln$。微分・積分で頻出
- **底 $10$**：常用対数 $\\log$。桁数を直接表す（$\\log_{10} 1000 = 3$ → $1000$ は $4$ 桁）

「**指数の世界**」と「**対数の世界**」は、$2$ つの言語で同じ景色を語ったもの。**$1$ つ知れば、もう $1$ つも自動的に分かる**——これがこの単元の核です。`,
  steps: [
    {
      id: "step1",
      position: 1,
...
$2^x$ の増え方を絵にしてみると、$x = 7$ あたりまではほぼ平らに見えるのに、$x = 10$ で **突然 $1024$ に達する**——これが「**指数関数的増加**」と呼ばれる現象です。

**指数法則の核心——「指数を足し算に置き換える」**

指数の最大の便利さは、**かけ算が足し算に化ける** こと：

$$b^m \\times b^n = b^{m+n}$$

なぜ？$2^3 \\times 2^4$ は「$2$ を 3 回かけたもの」×「$2$ を 4 回かけたもの」だから、合計 **$2$ を 7 回かけた数** $= 2^7$。**個数を足すだけ**。

| 法則 | 意味 |
|---|---|
| $b^m \\times b^n = b^{m+n}$ | 同じ底をかける → 指数を **足す** |
| $b^m \\div b^n = b^{m-n}$ | 同じ底を割る → 指数を **引く** |
| $(b^m)^n = b^{m \\times n}$ | 累乗の累乗 → 指数を **掛ける** |

**この系列で 9 問繰り返したこと**

Step 1〜9 では、$2^n, 3^n, 5^n, 4^n$ など、いろいろな底と指数の組合せをひたすら計算しました。手が馴染んでくると、$2^7 = 128$ や $3^3 = 27$ がぱっと出るようになります。

**Step 10：質を変える——「桁の話」へ**

Step 10 だけ、$10^3 = 1000$ という底 $10$ の問題が出ました。これは「**指数 $= $ 桁の数**」という、$10$ 進法ならではの特別な関係：
...
「**指数が桁の数を表す**」という発想は、対数のいちばん身近な使い方につながります。

────────

**もっと深く** — 指数の世界を広げる

**$0$ と 負と分数の指数**：指数法則 $b^m \\div b^n = b^{m-n}$ を **そのまま信じる** と、自然に拡張できます。

- $b^0 = b^{n-n} = b^n / b^n = 1$（だから何乗もしない数は $1$）
- $b^{-n} = b^{0-n} = 1 / b^n$（負の指数は逆数）
- $b^{1/2} \\times b^{1/2} = b^1 = b$ → $b^{1/2} = \\sqrt{b}$（分数の指数はルート）

これらは「**法則を保つために、定義を広げる**」という数学のよくあるやり方の好例です。

**身の回りの指数関数的増加**

- **複利**：年利 $5\\%$ なら $14$ 年で資産が $2$ 倍
- **感染症**：$1$ 人が $2$ 人にうつせば、$n$ 世代で $2^n$ 人
- **ムーアの法則**：コンピュータの性能が約 $1.5$ 年ごとに $2$ 倍
- **核分裂・連鎖反応**：$1$ 個の中性子が $2$ 個を生み、$2$ が $4$ に、$4$ が $8$ に…

「**急激な変化**」を見たら、その裏には指数があると疑うと、世界がよく見えるようになります。

**指数と対数は表裏一体**

$2^{10} = 1024$（指数）と $\\log_2 1024 = 10$（対数）は **同じ関係の 2 方向の書き方**：

- 指数：**底 $\\to$ 値**（「$2$ を $10$ 回かけたら？」→ $1024$）
- 対数：**値 $\\to$ 指数**（「$2$ を何回かけたら $1024$？」→ $10$ 回）

次の系列 [対数の入り口] では、この逆方向を歩きます。`,
  steps: [
...
export const ALGEBRA_2_SERIES_LIST: LearnerSeries[] = [
  ...
  /* ALGEBRA2_DOT_SERIES・ALGEBRA2_VEC_MAG_SERIES は数Ⅲ・C 第9章ベクトルユニットへ吸収（カタログ外・SERIES_REDIRECTS） */
