...
- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 第 $3$ 章「図形と方程式」の直線の単元構成（$2$ 点から傾き $\\to$ 点と傾きから $y$ 切片 $\\to$ 平行 $\\to$ 線分の中点 $\\to$ 垂直＝傾きの積が $-1$）を参考。問題の値はすべてオリジナル。
...
export const ADV_POINT_LINE_DISTANCE_SERIES: LearnerSeries = {
  ...
  steps: [
    ...
    },
    {
      ...
      variationFromPrevious: "same", compareWithStepId: "step2",
      hints: [
        { layer: 1, text: "前題と比べてみよう。何が同じで、何が違う？ 点が原点から、また普通の点に戻った——前題で楽ができていたところは、今回もそのまま楽できる？" },
        { layer: 2, text: "変わったのは点だけ。前題では原点だったおかげで消えていた部分が、今回は残る。" },
    ...
    },
    {
      ...
      variationFromPrevious: "same", compareWithStepId: "step3",
      hints: [
        { layer: 1, text: "前題と比べてみよう。何が同じで、何が違う？ 直線の係数の顔ぶれが今までと変わった。前題まで分母がきれいな数になっていたのは、たまたまだったのかな？" },
        { layer: 2, text: "変わったのは直線の係数だけ。$5, 12$ という組を見て、何か思い出さない？（ピタゴラス数）" },
    ...
    },
    {
      id: "step7", position: 7,
      questionText:
        "直線 $4x + 3y + c = 0$ と原点 $(0, 0)$ の距離が $2$ でした。$c$ の値はいくつでしょう？（ただし $c > 0$ とします）",
      answer: 10, unit: "", unknownLabel: "定数 c",
      variationFromPrevious: "inverse", compareWithStepId: "step6",
      hints: [
        { layer: 1, text: "前題と比べてみよう。直線の係数も点も、顔ぶれはそのまま。でも、わかっているものと聞かれているものが入れかわっている。前題で使った関係は、逆向きにも読めるかな？" },
        { layer: 2, text: "前題と変わったのは、空いている場所だけ。前題では距離が空いていて、今回は直線の式の中の $c$ が空いている。" },
        { layer: 3, text: "前題は $4x + 3y + 15 = 0$ と原点の距離を求めて $\\dfrac{|15|}{5} = 3$ だった。今回は距離のほうが $2$ とわかっていて $c$ が未知。同じ関係を $c$ について読むと $\\dfrac{|c|}{\\sqrt{16+9}} = \\dfrac{|c|}{5} = 2$、つまり $|c| = 10$。絶対値なので $c = 10$ と $c = -10$ の $2$ つが出る（原点をはさんで反対側に、同じだけ離れた平行な直線がもう $1$ 本ある）。問題文の「$c > 0$」がそのうち $1$ つを選んでいるので、$c = 10$。" },
      ],
      formulaPreview: "|c| / 5 = 2 → c = 10",
    },
    {
      id: "step8", position: 8,
      questionText:
        "点 $P$ は直線 $y = x$ 上にあり、**第 1 象限**（$x > 0$ かつ $y > 0$）にあります。$P$ と直線 $3x - 4y + 10 = 0$ の距離が $3$ のとき、$P$ の $x$ 座標はいくつでしょう？",
      answer: 25, unit: "", unknownLabel: "P の x 座標",
      variationFromPrevious: "plus_alpha", compareWithStepId: "step7",
      hints: [
        { layer: 1, text: "前題と比べてみよう。距離のほうが先にわかっていて、逆向きに求めるところは同じ。でも、探しているものに注文がひとつ付いている。前題で効いた読み方は、そのまま使える？" },
        { layer: 2, text: "前題と変わったのは $1$ つ——空いている場所が「直線の中の数」から「点」に移ったぶん、その点の居場所を縛る条件が付け足されている。" },
        { layer: 3, text: "前題では未知が $c$ ひとつだったので、そのまま関係式に入れられた。今回の未知は点の座標で $2$ つある——そこで「直線 $y = x$ 上」という条件を使って $P(t, t)$ と書くと、未知は $t$ ひとつになる。$\\dfrac{|3t - 4t + 10|}{\\sqrt{9+16}} = \\dfrac{|10 - t|}{5} = 3$ より $|10 - t| = 15$、つまり $t = 25$ か $t = -5$。前題と同じく $2$ つ出るが、$t = -5$ は点 $(-5, -5)$ で第 $3$ 象限。第 $1$ 象限という条件が $t = 25$ を選ぶ。" },
      ],
      formulaPreview: "|10 − t| / 5 = 3 → t = 25",
    },
    {
      id: "step9", position: 9,
      questionText: "直線 $y = 3$ と点 $(2, 7)$ の距離はいくつでしょう？",
      answer: 4, unit: "", unknownLabel: "距離 d",
      variationFromPrevious: "qualitative", compareWithStepId: "step6",
      hints: [
        { layer: 1, text: "Step 6 のように、また距離そのものを聞かれている。でも直線の書かれ方が今までと違う。何が同じで、何が違う？ そもそも横に寝た線と点のへだたりは、目で見るとどう測れそう？" },
        { layer: 2, text: "変わったのは直線の書かれ方だけ。今までの直線は $ax + by + c = 0$ の形だったが、今回はその形になっていない。" },
    ...
    },
    {
      id: "step10", position: 10,
      questionText:
        "3 点 $A(0, 0)$, $B(4, 0)$, $C(0, 3)$ で作る三角形 $ABC$ の面積はいくつでしょう？（直線 $BC$ は与えられていません）",
      answer: 6, unit: "", unknownLabel: "面積",
      variationFromPrevious: "composite", compareWithStepId: "step9",
      hints: [
        { layer: 1, text: "Step 1〜9 で使ってきた距離と、前の系列で作った「$2$ 点を通る直線の式」——今回はその合わせ技。しかも聞かれているものが距離ですらない。これまでの道具は、どこに化けそう？" },
        { layer: 2, text: "前題から変わったのは $2$ つ。聞かれているものが距離から面積になったことと、直線が与えられず自分で用意するところ。" },
        { layer: 3, text: "面積 $=$ 底辺 $\\times$ 高さ $\\div 2$。底辺を $BC$ とすると、高さは点 $A$ から直線 $BC$ までの距離——ここで今までの公式がそのまま効く。直線 $BC$ は $3x + 4y - 12 = 0$、$A$ との距離（高さ）は $\\dfrac{|-12|}{5} = \\dfrac{12}{5}$。$BC = \\sqrt{16+9} = 5$。面積 $= \\dfrac{1}{2}\\cdot5\\cdot\\dfrac{12}{5} = 6$。今回はたまたま直角三角形なので $\\dfrac{1}{2}\\cdot4\\cdot3$ でも同じ $6$ になる——$2$ つの道が同じ答えに着くのは、やり方が正しいことの確かめになる。" },
      ],
...
**6 問同じ計算を繰り返したら見えてくる——ピタゴラス数の魔法**

Step 1〜6 では、同じ公式を **6 回繰り返し** ました。気づきましたか？毎回、距離が **きれいな整数** で出てきます。これ、偶然ではありません。
...
**Step 7・8：公式を逆から読む——距離のほうが先にわかっているとき**

Step 1〜6 は「点と直線が与えられて、距離を出す」向きでした。でも公式は **等式** なので、**逆からも読めます**。

Step 7 は、直線 $4x + 3y + c = 0$ の $c$ だけが空いていて、原点との距離が $2$ とわかっている場面：

$$\\dfrac{|c|}{\\sqrt{4^2 + 3^2}} = \\dfrac{|c|}{5} = 2 \\quad \\Longrightarrow \\quad |c| = 10$$

ここで **絶対値が効いてきます**。$|c| = 10$ から出るのは $c = 10$ と $c = -10$ の **2 つ**。原点をはさんで反対側に、同じだけ離れた平行な直線がもう 1 本あるからです。だから逆向きに問うときは、**「どちら側か」を断らないと答えが 1 つに決まりません**（問題文の「ただし $c > 0$」はそのための断りです）。

Step 8 では、空いている場所が「直線の中の数」から「点」に移ります。点の座標は $x$ と $y$ の **2 つ** が未知——このままでは決まりません。そこで **条件を 1 つ足します**：「その点は直線 $y = x$ 上にある」。すると点は $(t, t)$ と書けて、未知は $t$ ひとつ：

$$\\dfrac{|3t - 4t + 10|}{5} = \\dfrac{|10 - t|}{5} = 3 \\quad \\Longrightarrow \\quad |10 - t| = 15 \\quad \\Longrightarrow \\quad t = 25,\\ -5$$

やはり $2$ つ出ます。$t = 25$ は点 $(25, 25)$、$t = -5$ は点 $(-5, -5)$——直線をはさんで両側に 1 つずつ。「第 1 象限にある」という条件が、そのうち 1 つを選び出しています。

**逆から読むと、絶対値が「2 つの答え」として顔を出す**——これは公式の欠陥ではなく、「直線の両側に同じだけ離れた場所がある」という図形の事実が、そのまま式に現れているだけです。距離は **向きを捨てて大きさだけ** を見る量——捨てたものは、逆に読んだときに戻ってきます。
...
**Step 10：合わせ技——3 つの点だけから、三角形の面積が出る**

最後に、問いの質と道具の数を同時に変えます。**「3 つの座標だけから、三角形の面積を計算で出す」**——できるでしょうか？
...
ここでは **2 つのことが同時に起きています**——聞かれているものが「距離」から「面積」に変わり、しかも肝心の直線 $BC$ が与えられず、$2$ 点から自分で立てなければならない。前の系列で作った「$2$ 点を通る直線の式」と、この系列の「点と直線の距離」が、ここで合流します。

座標 3 つから、計算だけで面積が出てくる——これが **座標幾何の威力**。点と直線の距離は、ここでは「**高さ**」の役を演じています。

（この三角形はたまたま直角三角形なので $\\dfrac{1}{2} \\cdot 4 \\cdot 3 = 6$ でも出ます。**同じ答えに $2$ つの道から着く** のは、やり方が正しいことの確かめ。直角でない三角形になったとたん、近道は消えて、上の手順だけが残ります。）
...
- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 第 3 章「図形と方程式」の単元構成（点と直線の距離 → 三角形の面積への応用）を参考。問題の値はすべてオリジナル。

────────

**問いに戻ると**

「垂線の足の座標を計算せずに、距離は出せないか？」——出せます。しかも、垂線の足を通らないからこそ、**逆にも読める** 道具になりました。

- **順に読む**（Step 1〜6・9）：点と直線を入れると、距離が出てくる
- **逆に読む**（Step 7）：距離が先にわかっていれば、直線の位置（$c$）が決まる
- **条件を足して読む**（Step 8）：距離＋「どこに乗っているか」で、点そのものが決まる
- **高さとして読む**（Step 10）：距離は、面積を出すための「高さ」に化ける

垂線の足を求める道を選んでいたら、この $4$ つの読み方はどれも重くなっていたはずです。**足を通らずに、式ひとつで「へだたり」を測る**——それがこの公式の正体でした。`,
};
  ...
  steps: [
    {
      ...
      hints: [
        ...
        { layer: 2, text: "原点から点 $(x,y)$ までのへだたりは、直角をはさむ $2$ 辺が $x$ と $y$ の直角三角形の斜辺。そのへだたりが半径にちょうど等しい、と書けたら？" },
    ...
    },
    {
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。中心の置かれ方も、式の形も、そのまま。変わったのは 1 か所だけ——それはどこ？ 前題で効いた見方は、そのまま使える？" },
        { layer: 2, text: "前題と変わったのは半径だけ。前題では半径 $3$ で $N = 9$ だった——$N$ は半径の何だった？" },
    ...
    },
    {
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。何が同じで、何が違う？ 前題で効いた「中心からのへだたりが一定」という見方は、今もそのまま使える？" },
        { layer: 2, text: "前題と変わったのは、中心が原点から動いたこと——その 1 点だけ。へだたりを測る相手が変われば、式のどこが変わる？" },
    ...
    },
    {
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。中心の置かれ方に、前題までとは違うところがある。何が同じで、何が違う？" },
        { layer: 2, text: "前題と変わったのは、中心の $y$ 座標が負になったこと——その 1 点だけ。負の位置からのへだたりを書くと、かっこの中の見た目はどうなる？" },
    ...
    },
    {
      ...
      position: 5,
      questionText:
        "円の方程式 $(x+3)^2 + (y-5)^2 = 10$ で表される円の半径はいくつでしょう？",
      answer: Math.sqrt(10),
      answerDisplay: "√10",
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。与えられているものと、聞かれているものが入れかわっていない？ 前題で効いた見方は、向きを変えても同じように効く？" },
        { layer: 2, text: "前題と変わったのは、式のほうが先に手元にあること——その 1 点だけ。前題で右辺に置いたものは、何だった？" },
        { layer: 3, text: "前題までは「中心と半径 → 式」の向きだった。今回はその逆で「式 → 半径」。右辺は半径の2乗なので $r^2 = 10$、よって $r = \\sqrt{10}$。中心 $(-3, 5)$ も式からそのまま読める。半径がきれいな整数でなくても、$r^2$ を通せば式はきれいなまま。" },
      ],
      formulaPreview: "r² = 10, r = √10",
    },
    {
      id: "step6",
      position: 6,
      ...
      hints: [
        { layer: 1, text: "ここで場面が変わった。前題までと比べて、与えられているものが違う。前題まで手がかりにしていたものは、今回はどこにも書かれていない——それでも円は 1 つに決まる？" },
        { layer: 2, text: "前題までと変わったのは、円が「$2$ 点の与えられ方」で決まっていること。この $2$ 点の間に横たわっているのは、円のどの部分？" },
    ...
    },
    {
      ...
      hints: [
        { layer: 1, text: "ここでまた場面が変わった。前題までと比べて、式の書かれ方が違う。これまで一目で読めていたものが、この書き方では隠れている——でも、まったくの別物だろうか？" },
        { layer: 2, text: "前題までと変わったのは、式がかっこを開いた形で書かれていること——その 1 点だけ。開かれたものは、もう一度かたまりに戻せない？" },
    ...
    },
    {
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。式の書かれ方は同じ仲間で、並んでいる数だけが違う。前題で効いた見方は、そのまま使える？" },
        { layer: 2, text: "前題と変わったのは、並んでいる数だけ。前題でたどった道を、そのままなぞれる？ 動かした分の出入りだけ気をつけて。" },
    ...
    },
    {
      ...
      position: 9,
      questionText:
        "方程式 $x^2 + y^2 - 6x + my - 27 = 0$ が表す円の中心が $x$ 軸上にあるとき、この円の半径はいくつでしょう？",
      answer: 6,
      unit: "",
      unknownLabel: "半径",
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。何が同じで、何が違う？ 前題には無かった一言が、うしろに付いている——その一言は、何を決めてくれそう？" },
        { layer: 2, text: "前題と変わったのは、数のひとつが文字のままで、そのかわりに条件がひとつ増えていること。その条件は、円のどこの位置についての話？" },
        { layer: 3, text: "前題までと同じように整えると $(x-3)^2 + \\left(y + \\dfrac{m}{2}\\right)^2 = 36 + \\dfrac{m^2}{4}$。中心は $\\left(3, -\\dfrac{m}{2}\\right)$。「中心が $x$ 軸上にある」は「中心の $y$ 座標が $0$」だから $-\\dfrac{m}{2} = 0$、すなわち $m = 0$。すると $(x-3)^2 + y^2 = 36$ となり、半径 $= 6$。" },
      ],
      formulaPreview: "m = 0, (x-3)² + y² = 36, r = 6",
    },
    {
      id: "step10",
      position: 10,
      questionText:
        "方程式 $x^2 + y^2 - 6x - 8y + k = 0$ が円を表すのは、$k$ がある値より小さいときだけです。その境目の値はいくつでしょう？",
      answer: 25,
      unit: "",
      unknownLabel: "境目の $k$",
      ...
      hints: [
        { layer: 1, text: "前題と比べてみよう。文字がひとつ混じっているのは同じ。でも今回は、与えられているものと聞かれているものが入れかわっていて、しかも「円を表すかどうか」まで話題になっている。前題までの見方は、そのまま届く？" },
        { layer: 2, text: "前題と変わったのは、円になるかどうかそのものが問われていること。これまで最後にいつも出てきた「右辺の値」は、どんな値でないと円にならない？" },
        { layer: 3, text: "前題までと同じように整えると $(x-3)^2 + (y-4)^2 = 25 - k$。右辺は半径の2乗にあたる値だから、正のときだけ円になる（$0$ なら点 $(3,4)$ ただ 1 つ、負なら満たす点は無い）。$25 - k > 0$ すなわち $k < 25$。だから境目は $k = 25$。" },
      ],
      formulaPreview: "(x-3)² + (y-4)² = 25 − k, 境目は k = 25",
    },
  ],
...
最初の $4$ 問は、与えられた中心と半径から **標準形を書く** 練習でした。気をつけるポイントだけ：

- 中心 $(a, b)$ の符号——たとえば $b = -2$ なら $(y - (-2))^2 = (y + 2)^2$
- 右辺は半径の **2 乗**——半径 $4$ なら $r^2 = 16$

**Step 5：同じ式を、逆から読む**

Step 5 は向きを裏返した問題でした。式 $(x+3)^2 + (y-5)^2 = 10$ が先に手元にあるとき、右辺の $10$ は $r^2$ なので、半径は $\\sqrt{10}$。中心 $(-3, 5)$ もそのまま読めます。

**作る向き（中心と半径 → 式）と、読む向き（式 → 中心と半径）は、同じ 1 本の翻訳の行き帰り**——これが標準形のもう半分の便利さ。半径がきれいな整数でなくても、$r^2$ を通せば式のほうはきれいなまま書ける、というおまけもついてきます。
...
**Step 9：条件が 1 つ増えると——文字の混じった一般形**

係数の $1$ つが文字のままでも、条件が $1$ つあれば円は決まります。$x^2 + y^2 - 6x + my - 27 = 0$ を平方完成すると：

$$(x - 3)^2 + \\left(y + \\dfrac{m}{2}\\right)^2 = 36 + \\dfrac{m^2}{4}$$

中心は $\\left(3, -\\dfrac{m}{2}\\right)$。「**中心が $x$ 軸上にある**」は「中心の $y$ 座標が $0$」ということなので $m = 0$。すると $(x-3)^2 + y^2 = 36$ で、半径は $6$ と決まります。

円は **中心 $(a, b)$ と半径 $r$ の $3$ つ** で決まる図形。だから **条件を $1$ つ足すたびに、自由度が $1$ つ減る**——同じ直線上に無い $3$ 点を通る円がただ $1$ つに決まるのも、同じ勘定です。

**Step 10：一般形は、いつでも円を表すとは限らない**

ここがこの単元でいちばん転びやすいところ。一般形 $x^2 + y^2 + lx + my + n = 0$ は **必ず円になるわけではありません**。平方完成したあとの右辺——$r^2$ にあたる値——が **正でなければ円にならない** からです。

$x^2 + y^2 - 6x - 8y + k = 0$ なら：

$$(x - 3)^2 + (y - 4)^2 = 25 - k$$

- $25 - k > 0$（$k < 25$）……半径 $\\sqrt{25-k}$ の **円**
- $25 - k = 0$（$k = 25$）……左辺が $0$ になるのは $1$ 点 $(3, 4)$ だけ（**点円**）
- $25 - k < 0$（$k > 25$）……左辺は $0$ 以上なので、満たす点は **1 つも無い**

だから境目は $k = 25$。**式が書けることと、その式が図形を表すことは別**——この区別は、この先の $2$ 次曲線でもずっと効いてきます。
...
- 標準形が与えられたら → 中心と半径をそのまま読む（作る向きの裏返し）
- 直径の両端のような変則的な情報でも → 中心と半径を取り出してから書く
- 一般形が与えられたら → 平方完成で標準形に戻して中心と半径を読む
- 文字が混じっていても → 条件を $1$ つ使えば決まる。ただし **$r^2$ にあたる値が正か** は最後に必ず確かめる
...
- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 第3章「図形と方程式」の単元構成（標準形 → 直径の両端 → 一般形と平方完成 → 円を表す条件）を参考。問題の値はすべてオリジナル。

────────

**問いに戻ると**

「中心からの距離が一定の点の集まりを、$1$ つの式で書けないか？」——書けます。しかも **たった $1$ 本**：

$$(x - a)^2 + (y - b)^2 = r^2$$

この式は「中心 $(a, b)$ からのへだたりが $r$」をそのまま書き写しただけ。だから、中心と半径が分かれば式が書けるし（Step 1〜4）、式があれば中心と半径が読める（Step 5）。直径の両端しか無くても（Step 6）、かっこを開いた形で渡されても（Step 7・8）、文字が混じっていても（Step 9）、やることは $1$ つ——**中心と半径を見つけにいく**。

そして最後にひとつ。その $r^2$ にあたる値が正でなければ、円は **無い**（Step 10）。**式は図形を書き写す道具であって、式が書けたことは図形があることの保証ではない**——ここまでが、この単元の景色です。`,
};
...
export const ADV_CIRCLE_LINE_SERIES: LearnerSeries = {
  ...
  title: "円と直線の位置関係",
  subtitle:
    "数Ⅱ・B「図形と方程式」より — 共有点の個数を距離 $d$ と半径 $r$ で判定し、逆に「接する」「共有点をもつ」から直線を決める 10 問。",
  patternId: "CR2",
  unit: "advanced",
  ...
  steps: [
    ...
    },
    {
      ...
      position: 5,
      questionText:
        "円 $x^2 + y^2 = 9$ と直線 $4x + 3y + k = 0$ が接するとき、$k$ の正の値はいくつでしょう？",
      answer: 15,
      unit: "",
      unknownLabel: "$k$ の正の値",
      variationFromPrevious: "inverse",
      compareWithStepId: "step4",
      hints: [
        { layer: 1, text: "前題と比べてみよう。ここまでは直線が全部決まっていて、共有点の個数を答えてきた。今回はその関係が裏返っている——何が分かっていて、何を探す？" },
        { layer: 2, text: "変わったのは 1 つ——前題は直線の側に分からない数が無く、個数のほうを答えた。今回は「接する」という位置関係のほうが先に分かっていて、直線の側に分からない数がある。" },
        { layer: 3, text: "使う道具は前題までと同じ——中心から直線までの距離 $d$ と半径 $r$。中心 $(0,0)$ から直線 $4x + 3y + k = 0$ までの距離は $d = \\dfrac{|k|}{\\sqrt{16+9}} = \\dfrac{|k|}{5}$、半径は $r = 3$。接するのは $d = r$ のときだから $\\dfrac{|k|}{5} = 3$、$|k| = 15$。接する直線は $k = 15$ と $k = -15$ の $2$ 本（円をはさんで反対側に $1$ 本ずつ）。正のほうを答えて $k = 15$。" },
      ],
      formulaPreview: "|k|/5 = 3 → k = ±15 → 正のほうは 15",
    },
    {
      id: "step6",
      position: 6,
      questionText:
        "円 $x^2 + y^2 = 9$ と直線 $4x + 3y + k = 0$ が共有点をもつような整数 $k$ は、全部で何個あるでしょう？",
      answer: 31,
      unit: "",
      unknownLabel: "整数 $k$ の個数",
      variationFromPrevious: "plus_alpha",
      compareWithStepId: "step5",
      hints: [
        { layer: 1, text: "前題と比べてみよう。円も直線も前題のまま。違うのは、探すものに条件が 1 つ足されていること——何が足された？" },
        { layer: 2, text: "変わったのは 1 つ——前題は $k$ をぴたりと決める条件だった。今回は $k$ に幅があり、その幅の中で整数のものを数える。" },
        { layer: 3, text: "$d = \\dfrac{|k|}{5}$、$r = 3$ は前題と同じ。共有点をもつのは「接する」か「交わる」、つまり $d \\le r$ のとき。$\\dfrac{|k|}{5} \\le 3$ より $|k| \\le 15$、すなわち $-15 \\le k \\le 15$。**接する $k = \\pm 15$ も共有点を $1$ 個もつので、両端を含める**（ここを外すと $2$ 個足りなくなる）。整数は $-15$ から $15$ までで $15 + 15 + 1 = 31$ 個。Step 1 の直線（$k = -20$）はこの幅の外にあった——だから共有点が無かった。" },
      ],
      formulaPreview: "|k| ≤ 15 → −15 ≤ k ≤ 15 → 整数は 31 個",
    },
    {
      id: "step7",
      position: 7,
      ...
      compareWithStepId: "step4",
      hints: [
        { layer: 1, text: "Step 4 までの「個数を答える」問題と比べてみよう。今回は円の中心が原点にない。距離を測るとき、何が変わる？" },
        { layer: 2, text: "変わったのは 1 つ——円の中心が原点から動いたこと。半径の読み方も、比べる相手も、Step 4 までと同じ。" },
    ...
    },
    {
      ...
      compareWithStepId: "step8",
      hints: [
        { layer: 1, text: "前題と比べてみよう。聞かれているもの（共有点の個数）は同じ。違うのは、通る道が問題文で指定されていること——その道は、Step 8 までの道と何が違う？" },
        { layer: 2, text: "変わったのは 1 つ——判定の相手が「中心から直線までの距離」から「共有点の $x$ 座標そのものを解にもつ方程式」に移ったこと。" },
        { layer: 3, text: "$x^2 + (2x-5)^2 = 5$ を整理すると $5x^2 - 20x + 20 = 0$、両辺を $5$ で割って $x^2 - 4x + 4 = 0$。$D/4 = (-2)^2 - 4 = 0$ なので実数解は $1$ 個（重解 $x=2$）、共有点も $1$ 個で接する。距離の道でも $d = \\dfrac{|-5|}{\\sqrt{5}} = \\sqrt{5} = r$ で接する——$2$ つの道が同じ答えに着く。" },
      ],
    ...
    },
    {
      ...
      position: 10,
      questionText:
        "直線 $y = 2x + k$ が円 $x^2 + y^2 = 5$ と異なる $2$ 点で交わるような整数 $k$ は、全部で何個あるでしょう？（判別式 $D$ で考えてみよう）",
      answer: 9,
      unit: "",
      unknownLabel: "整数 $k$ の個数",
      variationFromPrevious: "composite",
      compareWithStepId: "step9",
      hints: [
        { layer: 1, text: "前題と比べてみよう。円も直線も前題と同じ顔ぶれ。でも問われ方は 2 つ変わっている——何と何が入れ替わった？" },
        { layer: 2, text: "変わったのは 2 つ——Step 5 でやった「向きの入れ替え」と、Step 6 でやった「整数を数える」が、今度は判別式の道の上で同時に起きている。" },
        { layer: 3, text: "前題と同じ道で始める。$x^2 + (2x+k)^2 = 5$ を整理すると $5x^2 + 4kx + k^2 - 5 = 0$。異なる $2$ 点で交わるのは、この $2$ 次方程式が異なる $2$ 実数解をもつとき、つまり $D>0$。$D/4 = (2k)^2 - 5(k^2-5) = 25 - k^2 > 0$ より $k^2 < 25$、$-5 < k < 5$。**接する $k = \\pm 5$ は共有点 $1$ 個なので、ここでは端を含めない**（前題の $k=-5$ がちょうどその境目だった）。整数は $-4, -3, \\ldots, 4$ の $9$ 個。距離の道でも同じ：直線を $2x - y + k = 0$ と書くと $d = \\dfrac{|k|}{\\sqrt{5}}$、$r = \\sqrt{5}$、$d < r$ から $|k| < 5$ ——同じ幅に着く。" },
      ],
      formulaPreview: "D/4 = 25 − k² > 0 → −5 < k < 5 → 整数は 9 個",
    },
  ],
...
**Step 1〜4・7〜8：距離 $d$ で判定する練習**

これらの問題では、いろいろな円と直線について、距離 $d$ を計算して位置関係を判定しました。Step 1〜4 は **原点中心の円**、Step 7〜8 は **中心が原点でない円**。$d$ と $r$ の大小を比べるだけで、$0/1/2$ の答えが出てくる——同じ手順を繰り返すうちに、計算が手に馴染んできます。

**Step 5〜6：逆に読む——「接する」は境目**

Step 5 では、与えるものと求めるものが **入れ替わり** ました。直線 $4x + 3y + k = 0$ の $k$ が分からない代わりに、「**接する**」という位置関係のほうが先に与えられている。使う関係式は同じ $d = r$ ですが、こんどは $k$ について解く——**判定の式は、そのまま条件の式として読める**。

これが効くのは、$d = r$（接する）が **境目** だからです。$k$ を動かすと直線は平行に動き、$d$ は連続に変わる。だから「離れている／接する／交わる」の切り替わる瞬間＝境目さえ押さえれば、その両側がまるごと決まります。Step 6 はその読みで、**共有点をもつ $k$ の幅** を出し、そこに「整数だけ」という条件を $1$ つ足して数えました。境目の $k$ 自身も共有点を $1$ 個もつので、**端を含める**——ここが落とし穴です。

**Step 9〜10：もう $1$ つの道——質的変化と複合**

Step 9〜10 では、同じ問いを **別の方法（連立して判別式 $D$）** で解きました。

Step 9 は、円 $x^2 + y^2 = 5$ と直線 $y = 2x - 5$ について、$y$ を消去して $5x^2 - 20x + 20 = 0$ → $x^2 - 4x + 4 = 0$ → $D/4 = 0$ → 接する（$1$ 個）。方法 1 で計算しても、$d = \\dfrac{|-5|}{\\sqrt{5}} = \\sqrt{5} = r$ → 接する。**$2$ つの道が同じ答えに到達する**——「**$1$ つの真実に $2$ つの道**」を体感する場面です。

Step 10 は、その新しい道の上で Step 5 の **逆読み** と Step 6 の **整数を数える** を同時にやる問題でした。$D>0$ から $-5 < k < 5$。距離の道で解いても $d < r$ から $|k| < 5$ で、**同じ幅**。道が $2$ 本あるということは、**片方でもう片方を検算できる** ということでもあります。
...
**パラメータの範囲**：Step 6・Step 10 で扱ったのは、直線の **定数項** が動く場合（平行に動く直線）でした。**傾き $m$ のほうが動く**場合——「点 $(0, 5)$ を通る直線が円と $2$ 点で交わるような $m$ の範囲は？」——も、やることは同じ。$d < r$ または $D > 0$ の **不等式を $m$ について解く** だけ。方法 1 なら $d$ を $m$ で表す、方法 2 なら $D$ を $m$ で表す。ただし傾きが動くときは直線が円のまわりを回るので、$d$ の式が $m$ の分数式になり、扱いは少し重くなります。
...
連立＋判別式 $D$ という別の道もあり、同じ答えに到達する——でも **距離 $d$ ひとつ** で「絵を描かずに見抜く」ことができる。

そしてこの $3$ つの関係式は、**読む向きを変えれば、そのまま直線を決める条件になる**。$d = r$ は「接するかどうか」の判定であると同時に、「接するような直線はどれか」の条件式でもある——絵を描かずに見抜けるということは、絵を描かずに **作れる** ということでもあります。これがこの単元の到達点。`,
};
  ...
  steps: [
    {
      ...
      compareWithStepId: null,
      hints: [
        { layer: 1, text: "円の上に点 $P$ をとって、そこで円にちょうど触れる直線を思いうかべてみよう。$P$ の座標だけが分かっているとき、その直線の式は一行で書けないかな？ 円の式を、接点の座標で「読み替える」としたら、どんな形になりそう？" },
    ...
    },
    {
      ...
      position: 7,
      questionText:
        "円 $x^2 + y^2 = 25$ 上のある点 $P(a, b)$ における接線が、$6x + 8y = 50$ と書かれていました。接点の $x$ 座標 $a$ はいくつでしょう？",
      answer: 3,
      unit: "",
      unknownLabel: "接点の x 座標 a",
      variationFromPrevious: "inverse",
      compareWithStepId: "step6",
      hints: [
        { layer: 1, text: "ここで問いが逆向きになった。前題までは接点をもらって接線の式を書いた。今回は接線の式のほうが先に分かっていて、接点を当てる。前題までに見てきた形と、この式の見た目を比べてみよう——そのまま重ねられる？" },
        { layer: 2, text: "前題から変わったのは向きだけで、使う形は同じ。ただし公式の形なら、右辺は円の右辺と同じ数になっているはず。この式の右辺は、円の右辺と同じ数かな？" },
        { layer: 3, text: "公式の形は $ax + by = r^2$ で、右辺は $r^2 = 25$ のはず。ところが与えられた式の右辺は $50$。両辺を $2$ で割ると $3x + 4y = 25$ となって公式の形にそろう。係数を読むと $a = 3$、$b = 4$。検算 $3^2 + 4^2 = 25$ ✓ で接点は円上にある。接点から接線を書いてきた前題までを、そのまま逆向きに読んだだけ。" },
      ],
      formulaPreview: "6x + 8y = 50 → 3x + 4y = 25 → a = 3",
    },
    {
      id: "step8",
      position: 8,
    ...
    },
    {
      ...
      position: 10,
      questionText:
        "円 $x^2 + y^2 = 25$ の外側にある点 $Q$ から接線を $2$ 本引いたら、接点がちょうど $(3, 4)$ と $(4, -3)$ になりました。$Q$ の $x$ 座標はいくつでしょう？",
      answer: 7,
      unit: "",
      unknownLabel: "Q の x 座標",
      variationFromPrevious: "composite",
      compareWithStepId: "step9",
      hints: [
        { layer: 1, text: "step8・step9 と比べてみよう。あちらは外の点をもらって接点を探した。今回は分かっているものと求めるものが入れかわっている——step7 でやった逆向きの読み方が、この場面でも効かないかな？" },
        { layer: 2, text: "変わったのは向きだけ。$2$ つの接点はどちらも分かっているのだから、そこから書けるものが $2$ つある。その $2$ つと点 $Q$ は、どんな関係にある？" },
        { layer: 3, text: "接点 $(3,4)$ での接線は $3x + 4y = 25$、接点 $(4,-3)$ での接線は $4x - 3y = 25$（どちらも接点の座標を係数にするだけ）。$Q$ はこの $2$ 直線がともに通る点、つまり交点。連立して、第 $1$ 式 $\\times 3$ ＋ 第 $2$ 式 $\\times 4$ で $25x = 175$、$x = 7$（このとき $y = 1$）。検算：$Q(7,1)$ は $7^2+1^2=50 > 25$ なので確かに円の外。「外の点から $2$ 本」の場面（質的変化）を、接点から出発して逆向きにたどった複合の問題。" },
      ],
      formulaPreview: "3x+4y=25 と 4x−3y=25 の交点 → Q(7, 1)",
    },
  ],
...
Step 7 では向きが反転します。接線の式のほうが与えられて、接点を当てる。公式が「接点 → 接線」の一方通行ではなく、**両方向に読める等式**であることを確かめる $1$ 問です。

ただし落とし穴がひとつ。$6x + 8y = 50$ という式は、確かに円 $x^2+y^2=25$ の接線ですが、**係数をそのまま読んで接点 $(6, 8)$ とするのは誤り**です（$6^2 + 8^2 = 100 \\neq 25$）。公式の形 $ax + by = r^2$ は、**右辺が $r^2$ ちょうどのとき**にだけ「左辺の係数＝接点」が成り立つ。両辺を $2$ で割って $3x + 4y = 25$ にそろえてはじめて、接点 $(3, 4)$ が読める。**直線は係数を何倍しても同じ直線だが、接点が読めるのは $1$ つの倍率だけ**——検算 $a^2+b^2=r^2$ は、その倍率を選び直したかどうかを教えてくれます。
...
**Step 10：外の場面を、逆からたどる**

最後の $1$ 問は、この道を反対向きに歩きます。与えられるのは $2$ つの接点のほうで、求めるのは **外の点そのもの**。接点が分かっていれば接線は Step 1〜6 の公式で $1$ 行ずつ書けるので、あとは $2$ 直線の交点を出すだけ——円の問題が、$2$ 元 $1$ 次連立に落ちます。①は「外の点が接線上にある」という条件でしたが、それを $a, b$ について解けば接点が、$x_0, y_0$ について解けば外の点が出る。**同じ $1$ つの式を、どちらの文字について解くかだけの違い**です。
...
中心 $O$、接点、$Q$ で直角三角形ができる（接点で $90°$）ので、$|OQ|^2 = r^2 + \\ell^2$ から $\\ell$ が出る。Step 10 の $Q(7, 1)$ と円 $x^2+y^2=25$ なら $\\ell = \\sqrt{49 + 1 - 25} = 5$——接点までの長さが半径とちょうど同じ、という気持ちのよい配置でした。
...
$1$ つの公式は、接点が与えられたときに即座に接線を書く道具にも、接線から接点を読み取る道具にも、円外の点から接線を引くときの起点（$ax + by = r^2$ が通過する条件）にもなる。**「接線」という幾何の問いが、$2$ 元 $1$ 次・$2$ 次の連立に帰着する**——図形と方程式の真骨頂が、ここに現れています。`,
};
...
export const ADV_BUNDLE_SERIES: LearnerSeries = {
  ...
  steps: [
      ...
      hints: [
        {
          layer: 1,
          text:
            "交点の座標は、まだ求めていない。それでも束の式は、$k$ をどう選んでも交点を通ってくれる。では、手もとに残っている自由（$k$）を、点 $P$ のためにどう使う？",
        },
        {
          layer: 2,
          text:
    ...
    },
    {
      ...
      position: 7,
      questionText:
        "$2$ 円 $C_1: x^2 + y^2 - 8 = 0$ と $C_2: x^2 + y^2 - 4x - 2y + 4 = 0$ の $2$ 交点と、原点 $O(0, 0)$ を通る円。今回は束を、係数を $1$ つ増やした対称形 $\\alpha C_1 + \\beta C_2 = 0$ で書く。このままだと $\\alpha, \\beta$ の組は $1$ 通りに決まらないので、**$\\alpha = 3$ と決めておく**。このとき $\\beta$ はいくつでしょう？",
      answer: 6,
      unit: "",
      unknownLabel: "対称形の係数 $\\beta$",
      variationFromPrevious: "plus_alpha",
      compareWithStepId: "step6",
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう**。通したい図形の条件は、前題とどう違う？ そして、束の書き方のほうには何が増えた？",
        },
        {
          layer: 2,
          text:
            "増えたのは **束の係数の個数**——前題はひとつ、今はふたつ。ふたつあると組が決まらないので、$\\alpha$ の値を決める断りが $1$ つ足されている。足された断りは、答えを $1$ つに絞るためにどう効く？",
        },
        {
          layer: 3,
          text:
            "原点なら定数項だけ残る（Step 3 と同じ）：$C_1(0, 0) = -8$、$C_2(0, 0) = 4$。対称形も原点で $0$ になるはず：$3 \\cdot (-8) + \\beta \\cdot 4 = 0$ → $\\beta = 6$。前題までの形に直すと $k = \\beta / \\alpha = 2$——**係数を $1$ つ増やしても、図形を決めているのは比だけ**。では増やした甲斐はどこにあるか：$\\alpha = 0$ と置けば $C_2$ 自身が表せる。$C_1 + k C_2 = 0$ の形では、$k$ をどう動かしても $C_2$ には届かなかった。",
        },
      ],
      formulaPreview: "3·C₁(O) + β·C₂(O) = 0 → 3(−8) + 4β = 0 → β = 6（k = β/α = 2）",
    },
    {
      id: "step8",
      position: 8,
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題までと比べてみよう**。ここまでは「通したい点」が毎回そえられていた。今回はそれが無い——では、係数を決める手がかりはどこにありそう？",
        },
        {
          layer: 2,
          text:
            "変わったのは **課された条件** ——「点を通ること」ではなく「$2$ 乗の項が残らないこと」。この条件は、束の式のどの部分だけを見れば確かめられる？",
        },
        {
          layer: 3,
          text:
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう**。前題は係数のほうを探していた。今回はその係数がもう手もとにある——では、探すものはどちらへ移った？",
        },
        {
          layer: 2,
          text:
            "変わったのは **求めるもの** ——係数から、その係数を入れて現れる図形そのものへ。係数が決まった束の式は、もうただ $1$ 本に定まっている。その $1$ 本を、実際に書き出すとどうなる？",
        },
        {
          layer: 3,
          text:
            "$(x^2 + y^2 - 25) - (x^2 + y^2 - 6x - 8y) = 6x + 8y - 25 = 0$、すなわち $6x + 8y = 25$。$N = 25$。**$2$ 円の交点を一度も計算せずに、その $2$ 点を結ぶ直線が $1$ 行で出た**——前題で発見した $k = -1$ の威力。前題は「条件 → 係数」、今は「係数 → 図形」。**問いの向きが入れ替わっただけで、使っている束の式は同じ**。",
        },
      ],
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題までと比べてみよう**。ここまでは骨組みの $2$ 本が、はじめから並べて渡されていた。今回そえられているのは式が $1$ つだけ、しかも探すものも変わっている。それでも、これまでの見方はどこかに効きそう？",
        },
        {
          layer: 2,
          text:
            "変わり方が **$2$ つ重なっている**：ひとつは、束を組み立てる側から、渡された式の中に束を見抜く側へ移ったこと。もうひとつは、探すものが係数でも図形でもなく **点** になったこと。まず前のほうから——この式の中で、$a$ はどんな役をしている？",
        },
        {
          layer: 3,
          text:
            "$a (x + y - 2) + (x - y) = 0$ という形。これは **束 $L_1 + a L_2 = 0$ そのもの**（$L_1: x - y = 0$、$L_2: x + y - 2 = 0$、係数が $a$）。Step 1–4 で「束は $L_1, L_2$ の交点を必ず通る」と学んだのを **逆向きに** 使うと、$a$ によらず通る点 = この $2$ 直線の交点 $(1, 1)$。$x$ 座標は $1$。**「束を見抜く」と「向きを逆にする」が同時に起きている**——それでも要るのは、Step 1 で使った $1$ つの事実だけ。",
        },
      ],
...
この対称形が Step 7。係数を $1$ つ増やしても、**図形を決めているのは比 $\\beta / \\alpha$ だけ**で、取り出せる図形の数はほとんど変わらない。増やした甲斐はただ $1$ 点、$\\alpha = 0$ という「$k$ では届かなかった端」が手に入ることにある。**道具を広げるとき、何が本当に増えたのかを数えてみる**——束はその練習にもなっている。
...
export const ADV_LOCUS_SERIES: LearnerSeries = {
  ...
  steps: [
    ...
    },
    {
      ...
      position: 5,
      questionText:
        "点 $P$ が「原点 $O$ と点 $A(0, 8)$ からの距離の比が $OP : AP = k : 1$」（$k$ は正の数）の条件で動いたところ、軌跡は **中心 $(0, 9)$、半径 $3$ の円** になった。もとの条件の $k$ はいくつでしょう？",
      answer: 3,
      unit: "",
      unknownLabel: "比の値 k",
      variationFromPrevious: "inverse",
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう**。前題で与えられていたものと、求めたものは何だった？ 今はどちらが先に分かっている？",
        },
        {
          layer: 2,
          text:
            "変わったのは **向きだけ**——条件から円へ、ではなく、円から条件へ。円の上の点はどれも同じ条件を満たしている。中心と半径が分かっているなら、**いちばん見やすい位置にある点**はどこにありそう？",
        },
        {
          layer: 3,
          text:
            "前題は「比 → 円」の向きだった。今度は逆向き。中心 $(0, 9)$、半径 $3$ の円は $y$ 軸と $(0, 6)$、$(0, 12)$ で交わる。この $2$ 点は軌跡上の点だから、もとの条件を満たしているはず。$(0, 6)$ で測ると $OP = 6$、$AP = |8 - 6| = 2$ なので $OP : AP = 3 : 1$。$(0, 12)$ でも $OP = 12$、$AP = 4$ でやはり $3 : 1$（**どの点で測っても同じ**なのがアポロニウスの円）。式でも確かめられる：$\\sqrt{x^2 + y^2} = k \\sqrt{x^2 + (y - 8)^2}$ を $2$ 乗して整理すると中心の $y$ 座標は $\\dfrac{8 k^2}{k^2 - 1}$。これが $9$ に等しいから $8k^2 = 9k^2 - 9$、$k^2 = 9$、$k > 0$ より $k = 3$。",
        },
      ],
      formulaPreview: "軌跡上の点 (0, 6) で OP : AP = 6 : 2 → k = 3（8k²/(k²−1) = 9 でも同じ）",
    },
    {
      id: "step6",
      position: 6,
      ...
      hints: [
        {
          layer: 1,
          text:
            "**Step 4–5 は「$2$ つの距離の比が一定」の話だった。今は「等距離」。比べてみよう——比が $1 : 1$ になると、何が起きる？**",
        },
        {
          layer: 2,
          text:
    ...
    },
    {
      ...
      position: 7,
      questionText:
        "点 $P$ が「$2$ 点 $A(0, 0)$ と $B(6, 0)$ から等距離」で動くが、**さらに「$A$ からの距離が $5$」という条件も満たす**とする。このとき $P$ は $2$ 個の点に絞られる。そのうち $y$ 座標が正である方の、$y$ 座標はいくつでしょう？",
      answer: 4,
      unit: "",
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう**。$2$ 点も、等距離という条件も、そのまま。違うのは、条件がもう $1$ つ乗っていること。前題で出た図形は、そのぶんどうなる？",
        },
        {
          layer: 2,
          text:
            "変わったのは **条件が $1$ つ増えた** ことだけ。前題の答えの図形の上には点が無限にあるが、増えた条件はその中からどれを選ばせる？ 増えた条件だけを取り出すと、それは Step 1 で出会ったのと同じ仲間の条件。",
        },
        {
          layer: 3,
          text:
            "前題の条件だけなら軌跡は直線 $x = 3$（前題の答え $3$）。その上の点は $(3, y)$ と書ける。そこへ増えた条件 $AP = 5$ を入れると $\\sqrt{3^2 + y^2} = 5$、$2$ 乗して $9 + y^2 = 25$、$y^2 = 16$、$y = \\pm 4$。$y$ 座標が正の方をとって $y = 4$。**条件を $1$ つ足すと、軌跡は「直線（無限個の点）」から「$2$ 個の点」に絞られ、符号の指定でついに $1$ 点になる**。$y = 4$。",
        },
      ],
      formulaPreview: "x = 3 かつ AP = 5 → 9 + y² = 25 → y = ±4 → y > 0 で y = 4",
    },
    {
      id: "step8",
      position: 8,
...
**軌跡から「抜ける点」がある**：「軌跡を求める」とは、条件を満たす点を **すべて、それだけ** 集めること。だから式を出したあとに「その式を満たす点は、本当に全部が条件を満たすか」を確かめる一手が要る。たとえば「$2$ 点 $A, B$ から等距離で、しかも $3$ 点 $A, B, P$ が三角形をつくる」という条件なら、軌跡は [垂直二等分線] から **線分 $AB$ の中点 $1$ つを除いた図形**（中点では三角形がつぶれてしまう）。アポロニウスの円でも、比 $OP : AP$ と書いた時点で $AP \\neq 0$ を暗に仮定しているから、点 $A$ 自身は最初から候補の外にいる。**式は「$1$ 点だけ抜けている」ことを自分では教えてくれない**——$x, y$ の式に翻訳したときに落ちてしまった条件を、最後にもう一度拾い直す。ここが軌跡でいちばん間違えやすい場所。
...
export const ADV_PARAMETRIC_SERIES: LearnerSeries = {
  ...
  steps: [
    ...
    },
    {
      ...
      position: 5,
      questionText:
        "円 $(x - 5)^2 + (y + 1)^2 = 49$ の上を動く点を、媒介変数 $\\theta$ を使って $x = 5 + a \\cos\\theta, \\ y = -1 + a \\sin\\theta$ と表したい。$a > 0$ のとき、$a$ はいくつでしょう？",
      answer: 7,
      unit: "",
      unknownLabel: "係数 a",
      variationFromPrevious: "inverse",
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう**。前題は媒介変数表示のほうが先に与えられていた。今はどちらが与えられていて、どちらを求める？",
        },
        {
          layer: 2,
          text:
            "変わったのは **問う向き** だけ。前題では媒介変数表示から円の式へ進んだ。その道を逆向きにたどると、$a$ は円のどの量にあたる？",
        },
        {
          layer: 3,
          text:
            "前題は $x = 2 + 3 \\cos\\theta, y = 1 + 3 \\sin\\theta$ から $(x - 2)^2 + (y - 1)^2 = 9$ を作った。そこでは $\\cos\\theta$ の前の $3$ が半径、$(2, 1)$ が中心になっていた。今回はその道を逆にたどる：$(x - 5)^2 + (y + 1)^2 = 49$ は中心 $(5, -1)$、半径 $\\sqrt{49} = 7$ の円だから $x = 5 + 7 \\cos\\theta, y = -1 + 7 \\sin\\theta$、$a = 7$。（$a = -7$ でも同じ円を描ける——回る向きが逆になるだけ。だから $a > 0$ と断ってある。）**媒介変数表示と円の式の間の道は、両方向に歩ける**。",
        },
      ],
      formulaPreview: "中心 (5, −1)、半径² = 49 → a = 7",
    },
    {
      id: "step6",
      position: 6,
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう。前題では何が与えられて、何を聞かれていた？ 今回はどうだろう。**",
        },
        {
          layer: 2,
          text:
...
export const ADV_LINEAR_PROGRAMMING_SERIES: LearnerSeries = {
  ...
  steps: [
    ...
    },
    {
      ...
      position: 5,
      questionText:
        "領域 $D$ に条件をもう $1$ つ足した領域 $E : x \\geq 0, \\ y \\geq 0, \\ x + y \\leq 6, \\ x \\leq 4$ を考える。この $E$ 上で $z = -x + 2 y$ の **最小値** はいくつでしょう？",
      answer: -4,
      unit: "",
      unknownLabel: "z の最小値",
      variationFromPrevious: "plus_alpha",
      ...
      hints: [
        {
          layer: 1,
          text:
            "**前題と比べてみよう**。求めるもの（最小値）も $z$ の式も同じ。増えたものが $1$ つある——それは領域にどう効く？",
        },
        {
          layer: 2,
          text:
            "変わったのは、領域を決める条件が $1$ つ増えたこと——その $1$ つだけ。前題で最小になっていた頂点は、いまも領域の中に残っている？",
        },
        {
          layer: 3,
          text:
            "条件 $x \\leq 4$ が加わると、領域は三角形から四角形へ狭まる。頂点は $(0, 0), (4, 0), (4, 2), (0, 6)$（$x = 4$ と $x + y = 6$ の交点が $(4, 2)$）。ここで $z = -x + 2 y$ を比べると $0, -4, 0, 12$。**最小は $(4, 0)$ で $-4$**。前題で最小だった $(6, 0)$ は、いまは領域の外——**条件を $1$ つ足すと領域は狭まり、最適の場所は新しい境界へ押し戻される**。前題の頂点をそのまま使ってしまうのが、ここのつまずきどころ。",
        },
      ],
      formulaPreview: "頂点 (0, 0), (4, 0), (4, 2), (0, 6) で z = 0, -4, 0, 12 → 最小値 -4",
    },
    {
      id: "step6",
      position: 6,
      questionText:
        "足した条件を外して、はじめの領域 $D : x \\geq 0, \\ y \\geq 0, \\ x + y \\leq 6$ に戻す。この $D$ 上で $z = x^2 + y^2$ の **最大値** はいくつでしょう？",
      answer: 36,
      unit: "",
      unknownLabel: "z の最大値",
      variationFromPrevious: "qualitative",
      ...
      hints: [
        {
          layer: 1,
          text:
            "**Step 1–5 と比べてみよう。領域は前題と同じ $D$ に戻っている。では、変わったのはどこだろう？**",
        },
        {
          layer: 2,
          text:
...
**例 $3$：制約を $1$ 本足すと、最適の場所が移る**

同じ三角形領域に $x \\leq 4$ という制約をもう $1$ 本足すと、領域は四角形 $(0, 0), (4, 0), (4, 2), (0, 6)$ に狭まります。ここで $z = 2 x + y$ の最大値を頂点で比べると $0, 8, 10, 6$ → **$(4, 2)$ で $10$**。制約を足す前の $12$（頂点 $(6, 0)$）より小さい。

**条件を $1$ つ足すほど領域は狭まり、最適の場所は新しい境界へ移る**——足した制約が最適解を切り落とすとき、その制約は「効いている」といいます。どの制約が効いているかを読むのが、線形計画法の実務そのものです。
