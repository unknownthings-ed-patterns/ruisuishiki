...
export const ADV_LINE_EQUATION_SERIES: LearnerSeries = {
  ...
  title: "直線の方程式を読む",
  subtitle:
    "数Ⅱ・B「図形と方程式」より — 傾き・切片・中点・垂直、垂直×切片の複合まで 11 問。",
  patternId: "LN1",
  unit: "advanced",
  revelationLabel: "傾き＝yの差／xの差。中点＝平均。垂直＝傾きの積が −1",
  drivingQuestion:
    "たった 1 本の式 $y=mx+n$ が平面の直線を完全に決めてしまうのはなぜ？ 図形と式は、どこまで翻訳しあえる？",
  steps: [
    {
      id: "step1", position: 1,
    ...
    {
      id: "step11", position: 11,
      questionText:
        "$y = 2x + 7$ に **垂直** で、点 $(2, 5)$ を通る直線の **y 切片** はいくつでしょう？",
      ...
      hints: [
        { layer: 1, text: "step 9・10（垂直の傾き）と step 5・6（点を代入して切片）を思い出そう。今回はその合わせ技——垂直な直線の切片を求める。" },
        {
          layer: 2,
          text:
            "まず $y=2x+7$ に垂直な傾きを出す（step9-10）。次にその傾きの $y=mx+n$ に点 $(2,5)$ を代入して切片を出す（step5-6）。二つを順に使えない？",
        },
        { layer: 3, text: "$y=2x+7$ に垂直なら傾きは $-\\dfrac{1}{2}$（積が $-1$）。$y=-\\dfrac{1}{2}x+n$ に $(2,5)$ を代入して $5 = -1 + n$、$n = 6$。「垂直（質的変化）」＋「点代入で切片（＋α）」の複合。" },
      ],
      formulaPreview: "m'=−1/2; 5 = −1/2·2 + n → n = 6",
    },
  ],
  derivation: `**中心の問い** ｜ たった 1 本の式 $y=mx+n$ が平面の直線を完全に決めてしまうのはなぜ？ 図形と式は、どこまで翻訳しあえる？

────────

**地図に 2 つの目印があれば、その 2 点を結ぶ「道」がぴったり 1 本に決まる。**
...
「たった 1 本の式 $y=mx+n$ が平面の直線を完全に決めてしまうのはなぜ？」——$m$（[傾き]）が直線の「向き」を、$n$（[切片]）が「位置」を、それぞれ 1 つに固定するから。そして傾き・切片・[中点]・[垂直]・平行は、すべて **座標の計算だけ**で——定規もコンパスも使わずに——読める。**図形と式は翻訳しあえる**。これが「図形と方程式」という単元名の意味であり、円も軌跡も領域も、この翻訳の上に建つ。`,
};
...
export const ADV_NUMBER_LINE_SERIES: LearnerSeries = {
  ...
  drivingQuestion:
    "直線上の点の位置は、座標の『差』と『重み付き平均』だけで——定規を当てずに——どこまで言い当てられる？",
  steps: [
    {
      id: "step1",
      position: 1,
    ...
    },
    {
      ...
      hints: [
        ...
        { layer: 3, text: "内分は「遠い方の比」を重みにする加重平均。文字で書くと、$A(x_1), B(x_2)$ を $m:n$ に分ける点は $\\dfrac{n x_1 + m x_2}{m + n}$——遠い方の比をクロスして掛けて足す。$2:3$ なら $\\dfrac{3 \\cdot 2 + 2 \\cdot 7}{2 + 3} = 4$。" },
      ],
    ...
    },
    {
      ...
      hints: [
        ...
        },
        {
          layer: 2,
          text:
            "外分の点は線分の外。内分の出し方をそっくり使えないかな——比の使い方の「どこか一箇所」を変えると、内側が外側になりそう。どこを変える？",
        },
        { layer: 3, text: "内分は $\\dfrac{n x_1 + m x_2}{m + n}$ だった。外分はこの式の片方の比を「負」にするだけ：$\\dfrac{-n x_1 + m x_2}{m - n}$。$3:1$ なら $\\dfrac{-1 \\cdot 2 + 3 \\cdot 8}{3 - 1} = 11$。内分と外分は別物でなく、符号ひとつ違いの兄弟。" },
      ],
  ...
  derivation: `**中心の問い** ｜ 直線上の点の位置は、座標の『差』と『重み付き平均』だけで——定規を当てずに——どこまで言い当てられる？
...
「直線上の点の位置は、座標の『差』と『重み付き平均』だけで——どこまで言い当てられる？」——その答えはすべて **座標の差と重み付き平均** から出る。

- 距離 $= |x_2 - x_1|$（座標の差の絶対値）
- [中点] $= (x_1 + x_2)/2$（座標の平均）
- [内分] $= (nx_1 + mx_2)/(m+n)$（[重み付き平均]、遠い方の比が重み）
- [外分] $= (-nx_1 + mx_2)/(m-n)$（[内分]の $n$ を $-n$ に置き換え、重みが負になる）

$4$ つの公式に見えるが、根は **$1$ つの考え方——「重み付き平均」**。重みを $1:1$ にすれば [中点]、一般の $m:n$ にすれば [内分]、片方を負にすれば [外分]。これからの「平面上の点」も同じ発想で広がる。
