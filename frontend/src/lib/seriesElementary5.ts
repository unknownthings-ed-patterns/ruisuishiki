...
学校で「**速さ × 時間 = 道のり**」を覚えるとき、「**は・じ・き**」（速さ・時間・距離）や「**み・は・じ**」（道のり・速さ・時間）と呼ぶ図を使います。
...
求めたいものを **隠す** と、残りの 2 つの計算が出てくる仕組み。実用的な覚え方として広く使われています。
...
平行四辺形の **片端を切って、反対側にくっつける** と、ちょうど **長方形** になります：
...
切り取った三角形を反対側に持っていくと、ぴったり長方形ができる——これが「面積 = 底辺 × 高さ」の理由。
