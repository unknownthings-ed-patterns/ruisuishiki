...
- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 章構成（一般角と三角関数・単位円による定義）を参考。問題の値はすべてオリジナル。
...
- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 章構成（弧度法・度との換算・扇形の弧長）を参考。問題の値はすべてオリジナル。
...
- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 章構成（三角方程式・不等式・範囲と解）を参考。問題の値はすべてオリジナル。
  ...
  steps: [
    ...
    },
    {
      ...
      hints: [
        {
          layer: 1,
          text: "前題と比べてみよう。与えられた和 $\\sin\\theta + \\cos\\theta$ は同じで、求めるものが**積**でなく**差**に変わった。前題で効いた手は、差の形でもそのまま効きそう？ ただし最後に $\\pm$ の二択が残る——それを決める材料として、今回は範囲が足されているよ。",
        },
        {
          layer: 2,
  ...
  steps: [
    ...
    },
    {
      ...
      hints: [
        {
          layer: 1,
          text: "step1 と比べてみよう。公式は同じだけれど、部品が足りない——右辺には $\\cos\\alpha$ と $\\sin\\beta$ も要るのに、与えられていない。足りない部品を自力で作る道具を、相互関係の系列で身につけていなかった？",
        },
        {
          layer: 2,
        ...
        {
          layer: 3,
          text: "[相互関係] より $\\cos\\alpha = \\sqrt{1 - \\dfrac{16}{25}} = \\dfrac{3}{5}$、$\\sin\\beta = \\sqrt{1 - \\dfrac{25}{169}} = \\dfrac{12}{13}$（第 $1$ 象限なので正）。組み立てて $\\sin(\\alpha+\\beta) = \\dfrac{4}{5}\\cdot\\dfrac{5}{13} + \\dfrac{3}{5}\\cdot\\dfrac{12}{13} = \\dfrac{20 + 36}{65} = \\dfrac{56}{65}$。**加法定理と相互関係の合流**——相互関係の系列の道具が、部品工場として働く。",
        },
      ],
...
**ここが胚細胞**：**特殊化と逆読みが、新しい公式を生む**。公式集を増やしたのではなく、$1$ 本の定理を「特別な場合に絞る」「後ろから読む」という $2$ つの操作で耕した。$3$ 倍角が欲しければ $\\sin(2\\theta+\\theta)$ に加法定理をもう $1$ 回——何倍角でも、**加法定理を繰り返せば必ず導けます**。覚える公式の数より、生やす手つきを持つこと。
...
「覚えていないのに公式が生えてくるのはなぜ？」——[加法定理] が**一般**の定理だから。一般は特殊を含む：$\\beta = \\alpha$ と絞れば倍角が生え、[相互関係] を差し込めば $\\cos 2\\theta$ の $3$ つの顔が生え、逆から読めば半角と次数下げが生える。公式は丸暗記するものではなく、$1$ 本の屋台骨（加法定理）から**その場で生やすもの**。「前から読む・後ろから読む・特別な場合に絞る」——この $3$ つの手つきが、公式集を $1$ 本の定理に畳んでしまいます。`,
};

/** TRIG9: 三角関数の合成（2 つの波を 1 つに＝加法定理の逆読み）。
 *  r・α・最大最小を別々の数値 step に分解して問う（D3——式全体は答えさせない）。
 *  三角関数ユニットの最終系列。系3（方程式）・系5（振幅・位相）・系7（加法定理）が合流する。
 *  背骨: docs/trig_continuation_series_design_fable.md §7（Fable 5 設計・Fable 5 実装）。 */
export const TRIG_COMPOSITION_SERIES: LearnerSeries = {
  id: "trig_composition_01",
  title: "三角関数の合成（2つの波は、1つの波に戻る）",
  subtitle:
    "数Ⅱ・B 三角関数より — $a\\sin\\theta + b\\cos\\theta$ を $1$ つの波にまとめる。点 $(a,b)$ の長さと向きが振幅と位相になる $10$ 問。",
  patternId: "TRIG9",
  unit: "algebra_2",
  revelationLabel:
    "$a\\sin\\theta + b\\cos\\theta = r\\sin(\\theta+\\alpha)$——ずれた 2 つの波の和は、また 1 つの波。振幅 $r$ と位相 $\\alpha$ は、点 $(a, b)$ の「長さ」と「向き」そのもの",
  drivingQuestion:
    "ずれた $2$ つの波 $\\sin\\theta$ と $\\cos\\theta$ を足しても、ばらばらにならず、また $1$ つのきれいな波に戻るのはなぜ？——係数の組 $(a, b)$ を平面の $1$ 点と見ると、その『長さ』と『向き』が、新しい波の高さとずれそのものだとしたら？",
  steps: [
    {
      id: "step1",
      position: 1,
      questionText:
        "$\\sin\\theta + \\cos\\theta$ は $2$ つの波の足し算です。これを $1$ つの波 $r\\sin(\\theta + \\alpha)$ の形にまとめることを**合成**といいます。[加法定理] でひらいた形 $r\\sin\\theta\\cos\\alpha + r\\cos\\theta\\sin\\alpha$ と見比べて、まず**振幅 $r$**（$r > 0$）を求めましょう。",
      answer: Math.sqrt(2),
      answerDisplay: "√2",
      inputAffordances: ["sqrt"],
      unit: "",
      unknownLabel: "sinθ+cosθ の合成の r",
      variationFromPrevious: null,
      compareWithStepId: null,
      hints: [
        {
          layer: 1,
          text: "見比べると、$\\sin\\theta$ の係数どうし・$\\cos\\theta$ の係数どうしが対応する：$r\\cos\\alpha = 1$、$r\\sin\\alpha = 1$。この $2$ つの数を座標に持つ点 $P(1, 1)$ を描くと、$r$ は図のどの長さにあたる？",
        },
        {
          layer: 2,
          text: "$r\\cos\\alpha$ と $r\\sin\\alpha$ が $(1, 1)$——これは「原点から長さ $r$・角 $\\alpha$ の方向に進んだ点の座標」の形（[単位円] の $r$ 倍）。つまり $r$ は原点から点 $(1, 1)$ までの**距離**。[三平方の定理] で出せるね。",
        },
        {
          layer: 3,
          text: "$r$ は原点から $P(1, 1)$ までの距離なので $r = \\sqrt{1^2 + 1^2} = \\sqrt{2}$。一般に $a\\sin\\theta + b\\cos\\theta$ の振幅は $r = \\sqrt{a^2 + b^2}$——**$\\sin$ の係数を $x$ 座標・$\\cos$ の係数を $y$ 座標にした点までの距離**。$2$ つの波の和が $1$ つの波に戻るのは、[加法定理] を後ろから読んでいるから。",
        },
      ],
      formulaPreview: "P(1,1) までの距離 r = √(1²+1²) = √2",
      figureMarker: "<<TRIG_COMPOSITION_POINT>>",
    },
    {
      id: "step2",
      position: 2,
      questionText:
        "同じ式 $\\sin\\theta + \\cos\\theta = \\sqrt{2}\\sin(\\theta + \\alpha)$ の**位相 $\\alpha$**（$-\\pi < \\alpha \\le \\pi$）を求めましょう。",
      answer: Math.PI / 4,
      answerDisplay: "π/4",
      inputAffordances: ["pi"],
      unit: "",
      unknownLabel: "sinθ+cosθ の合成の α",
      variationFromPrevious: "same",
      compareWithStepId: "step1",
      hints: [
        {
          layer: 1,
          text: "前題と比べてみよう。同じ点 $P(1, 1)$ の、今度は「向き」を読む番。$\\alpha$ は直線 $OP$ が $x$ 軸の正の向きとなす角——$P(1, 1)$ の方向は、どの特別角？",
        },
        {
          layer: 2,
          text: "前題と変わったのは、読むのが「長さ」から「向き」になっただけ。$r\\cos\\alpha = 1$・$r\\sin\\alpha = 1$ で $r = \\sqrt{2}$ だから、$\\cos\\alpha$ と $\\sin\\alpha$ が同じ値になる角——$45°$ 方向だね。ラジアンでは？",
        },
        {
          layer: 3,
          text: "$\\cos\\alpha = \\dfrac{1}{\\sqrt{2}}$、$\\sin\\alpha = \\dfrac{1}{\\sqrt{2}}$ となる角は $\\alpha = \\dfrac{\\pi}{4}$。よって $\\sin\\theta + \\cos\\theta = \\sqrt{2}\\sin\\left(\\theta + \\dfrac{\\pi}{4}\\right)$。検算に [加法定理] でひらくと $\\sqrt{2}\\left(\\sin\\theta\\cdot\\dfrac{1}{\\sqrt{2}} + \\cos\\theta\\cdot\\dfrac{1}{\\sqrt{2}}\\right) = \\sin\\theta + \\cos\\theta$——ちゃんと元に戻る。",
        },
      ],
      formulaPreview: "P(1,1) の向き = π/4 → √2 sin(θ+π/4)",
    },
    {
      id: "step3",
      position: 3,
      questionText:
        "係数を一般にします。$3\\sin\\theta + \\sqrt{3}\\cos\\theta$ を $r\\sin(\\theta+\\alpha)$ に合成したときの $r$（$r > 0$）を求めましょう。",
      answer: 2 * Math.sqrt(3),
      answerDisplay: "2√3",
      inputAffordances: ["sqrt"],
      unit: "",
      unknownLabel: "3sinθ+√3cosθ の r",
      variationFromPrevious: "plus_alpha",
      compareWithStepId: "step1",
      hints: [
        {
          layer: 1,
          text: "step1 と比べてみよう。手は同じ「係数の組を点にして距離を測る」。変わったのは点の場所だけ——今回の点はどこ？",
        },
        {
          layer: 2,
          text: "step1 と変わったのは係数が $(1, 1)$ から $(3, \\sqrt{3})$ になっただけ。原点からの距離を [三平方の定理] で測ると、根号の中はいくつになる？",
        },
        {
          layer: 3,
          text: "$P(3, \\sqrt{3})$ までの距離は $r = \\sqrt{3^2 + (\\sqrt{3})^2} = \\sqrt{9 + 3} = \\sqrt{12} = 2\\sqrt{3}$。係数がどんな値でも、振幅は $\\sqrt{a^2 + b^2}$ の一本道。",
        },
      ],
      formulaPreview: "r = √(9+3) = √12 = 2√3",
    },
    {
      id: "step4",
      position: 4,
      questionText:
        "同じ式 $3\\sin\\theta + \\sqrt{3}\\cos\\theta = 2\\sqrt{3}\\sin(\\theta+\\alpha)$ の $\\alpha$（$-\\pi < \\alpha \\le \\pi$）を求めましょう。",
      answer: Math.PI / 6,
      answerDisplay: "π/6",
      inputAffordances: ["pi"],
      unit: "",
      unknownLabel: "3sinθ+√3cosθ の α",
      variationFromPrevious: "same",
      compareWithStepId: "step2",
      hints: [
        {
          layer: 1,
          text: "step2 と比べてみよう。同じ「点の向きを読む」。今回の点の方向は、傾きで考えると分かりやすい——傾き（縦 ÷ 横）はいくつで、それはどの特別角の $\\tan$ だったかな？",
        },
        {
          layer: 2,
          text: "step2 と変わったのは点の場所だけ。直線 $OP$ の傾きは $\\dfrac{\\sqrt{3}}{3} = \\dfrac{1}{\\sqrt{3}}$——$\\tan\\alpha = \\dfrac{1}{\\sqrt{3}}$ となる第 $1$ 象限の角は？",
        },
        {
          layer: 3,
          text: "$\\tan\\alpha = \\dfrac{\\sqrt{3}}{3} = \\dfrac{1}{\\sqrt{3}}$ で $P$ は第 $1$ 象限だから $\\alpha = \\dfrac{\\pi}{6}$。よって $3\\sin\\theta + \\sqrt{3}\\cos\\theta = 2\\sqrt{3}\\sin\\left(\\theta + \\dfrac{\\pi}{6}\\right)$。$r$ は距離・$\\alpha$ は向き——点 $P$ が $2$ つとも教えてくれる。",
        },
      ],
      formulaPreview: "tanα = √3/3 = 1/√3・第1象限 → α = π/6",
    },
    {
      id: "step5",
      position: 5,
      questionText:
        "合成の**使い道**です。$y = 3\\sin\\theta + \\sqrt{3}\\cos\\theta$（$\\theta$ は全実数）の**最大値**と**最小値**を、カンマで区切って $2$ つとも答えましょう。",
      answer: 2 * Math.sqrt(3),
      answerDisplay: "2√3, −2√3",
      solutionSet: [2 * Math.sqrt(3), -2 * Math.sqrt(3)],
      inputAffordances: ["sqrt", "multi"],
      unit: "",
      unknownLabel: "3sinθ+√3cosθ の最大値と最小値",
      variationFromPrevious: "plus_alpha",
      compareWithStepId: "step4",
      hints: [
        {
          layer: 1,
          text: "前題までで、この式は $1$ つの波にまとまった。まとまってしまえば、[三角関数のグラフ] でやった「[振幅] が波の高さを決める」がそのまま使える——$1$ つの波の最大値・最小値は何で決まる？",
        },
        {
          layer: 2,
          text: "新しいのは「合成してから範囲を読む」こと。$y = 2\\sqrt{3}\\sin\\left(\\theta + \\dfrac{\\pi}{6}\\right)$ で、$\\sin(\\text{中身})$ は $-1$ から $1$ まで動く。全体はその何倍？",
        },
        {
          layer: 3,
          text: "$-1 \\le \\sin\\left(\\theta + \\dfrac{\\pi}{6}\\right) \\le 1$ の全体を $2\\sqrt{3}$ 倍して $-2\\sqrt{3} \\le y \\le 2\\sqrt{3}$。最大値 $2\\sqrt{3}$・最小値 $-2\\sqrt{3}$。**$2$ つの波のままでは山の高さが読めないが、$1$ つにまとめれば振幅がそのまま答えになる**——これが合成の最大の使い道。",
        },
      ],
      formulaPreview: "y = 2√3 sin(θ+π/6) → 最大 2√3・最小 −2√3",
    },
    {
      id: "step6",
      position: 6,
      questionText:
        "係数に**負**が入ります。$-\\sin\\theta + \\cos\\theta = \\sqrt{2}\\sin(\\theta+\\alpha)$ の $\\alpha$（$-\\pi < \\alpha \\le \\pi$）を求めましょう。（点 $P$ はもう第 $1$ 象限にいません）",
      answer: (3 * Math.PI) / 4,
      answerDisplay: "3π/4",
      inputAffordances: ["pi"],
      unit: "",
      unknownLabel: "−sinθ+cosθ の α",
      variationFromPrevious: "qualitative",
      compareWithStepId: "step4",
      hints: [
        {
          layer: 1,
          text: "step4 と比べてみよう。手は同じ「点の向きを読む」。でも今回の点は $P(-1, 1)$——$x$ 座標が負で、第 $2$ 象限にいる。向きの角は、もう鋭角ではなさそうだね。どのあたりの角になる？",
        },
        {
          layer: 2,
          text: "step4 と変わったのは「$P$ が第 $1$ 象限を出た」こと。$\\cos\\alpha = \\dfrac{-1}{\\sqrt{2}}$・$\\sin\\alpha = \\dfrac{1}{\\sqrt{2}}$——$\\cos$ が負で $\\sin$ が正の角は第 $2$ 象限。[一般角] の系列でやった「座標から角を読む」がここで効く。",
        },
        {
          layer: 3,
          text: "$P(-1, 1)$ は第 $2$ 象限で、$\\cos\\alpha = -\\dfrac{1}{\\sqrt{2}}$、$\\sin\\alpha = \\dfrac{1}{\\sqrt{2}}$ となる角は $\\alpha = \\dfrac{3\\pi}{4}$。よって $-\\sin\\theta + \\cos\\theta = \\sqrt{2}\\sin\\left(\\theta + \\dfrac{3\\pi}{4}\\right)$。**傾き（$\\tan$）だけで角を決めると象限を間違える**——$\\cos\\alpha$・$\\sin\\alpha$ の符号（＝点の象限）まで見るのが作法。",
        },
      ],
      formulaPreview: "P(−1,1)・第2象限 → α = 3π/4",
      figureMarker: "<<TRIG_COMPOSITION_POINT_Q2>>",
    },
    {
      id: "step7",
      position: 7,
      questionText:
        "逆向きに読みます。$\\sqrt{2}\\sin\\left(\\theta + \\dfrac{\\pi}{3}\\right)$ を $a\\sin\\theta + b\\cos\\theta$ の形にひらいたときの、$\\cos\\theta$ の係数 $b$ を求めましょう。",
      answer: Math.sqrt(6) / 2,
      answerDisplay: "√6/2",
      inputAffordances: ["sqrt"],
      unit: "",
      unknownLabel: "√2 sin(θ+π/3) の cosθ の係数",
      variationFromPrevious: "inverse",
      compareWithStepId: "step1",
      hints: [
        {
          layer: 1,
          text: "step1 と比べてみよう。step1 は「ひらいた形から $r$ と $\\alpha$ へ」まとめる向きだった。今度はその**逆**——まとまった形をひらく。ひらく道具は、この章の屋台骨のあの定理だね。どうひらける？",
        },
        {
          layer: 2,
          text: "step1 と変わったのは矢印の向きだけ。[加法定理] で $\\sin\\left(\\theta + \\dfrac{\\pi}{3}\\right) = \\sin\\theta\\cos\\dfrac{\\pi}{3} + \\cos\\theta\\sin\\dfrac{\\pi}{3}$ とひらいて、全体を $\\sqrt{2}$ 倍。$\\cos\\theta$ に付く係数はどの部分？",
        },
        {
          layer: 3,
          text: "$\\sqrt{2}\\sin\\left(\\theta + \\dfrac{\\pi}{3}\\right) = \\sqrt{2}\\left(\\sin\\theta \\cdot \\dfrac{1}{2} + \\cos\\theta \\cdot \\dfrac{\\sqrt{3}}{2}\\right) = \\dfrac{\\sqrt{2}}{2}\\sin\\theta + \\dfrac{\\sqrt{6}}{2}\\cos\\theta$。よって $b = \\dfrac{\\sqrt{6}}{2}$。合成（まとめる）と加法定理（ひらく）は互いの逆演算——因数分解と展開の関係と同じ。",
        },
      ],
      formulaPreview: "√2 sin(θ+π/3) = (√2/2)sinθ + (√6/2)cosθ → b = √6/2",
    },
    {
      id: "step8",
      position: 8,
      questionText:
        "畳む先を変えます。$\\sin\\theta + \\sqrt{3}\\cos\\theta$ を **$\\cos$ の波** $r\\cos(\\theta - \\beta)$ の形にまとめたときの $\\beta$（$-\\pi < \\beta \\le \\pi$）を求めましょう。（$r = 2$ です）",
      answer: Math.PI / 6,
      answerDisplay: "π/6",
      inputAffordances: ["pi"],
      unit: "",
      unknownLabel: "sinθ+√3cosθ = 2cos(θ−β) の β",
      variationFromPrevious: "qualitative",
      compareWithStepId: "step7",
      hints: [
        {
          layer: 1,
          text: "前題と比べてみよう。ひらく道具は同じ加法定理——ただし今度は $\\cos$ の加法定理で $\\cos(\\theta-\\beta)$ をひらいて見比べる。$\\cos$ の差の公式をひらくと、$\\sin\\theta$ と $\\cos\\theta$ の係数はそれぞれ何になる？",
        },
        {
          layer: 2,
          text: "新しいのは「畳む先が $\\sin$ でなく $\\cos$」なこと。$r\\cos(\\theta-\\beta) = r\\cos\\theta\\cos\\beta + r\\sin\\theta\\sin\\beta$ と見比べると、今度は $\\sin$ の係数が $r\\sin\\beta$・$\\cos$ の係数が $r\\cos\\beta$——step1 と役割が入れ替わる。$2\\sin\\beta = 1$・$2\\cos\\beta = \\sqrt{3}$ になる $\\beta$ は？",
        },
        {
          layer: 3,
          text: "$\\cos(\\theta-\\beta)$ をひらいて係数を見比べると $\\sin\\beta = \\dfrac{1}{2}$、$\\cos\\beta = \\dfrac{\\sqrt{3}}{2}$——$\\beta = \\dfrac{\\pi}{6}$。つまり $\\sin\\theta + \\sqrt{3}\\cos\\theta = 2\\cos\\left(\\theta - \\dfrac{\\pi}{6}\\right)$。同じ式は $2\\sin\\left(\\theta + \\dfrac{\\pi}{3}\\right)$ とも書ける——**$\\sin$ 合成と $\\cos$ 合成は同じ波の $2$ つの表記**（$\\cos$ は $\\sin$ の [位相] ずれだから）。",
        },
      ],
      formulaPreview: "2cos(θ−β) = 2cosθcosβ+2sinθsinβ → sinβ=1/2, cosβ=√3/2 → β=π/6",
    },
    {
      id: "step9",
      position: 9,
      questionText:
        "方程式との合流です。$0 \\le \\theta < 2\\pi$ で、$\\sqrt{3}\\sin\\theta + \\cos\\theta = 1$ を満たす $\\theta$ を**すべて**求めましょう（複数あるので「,」で区切って。$0$ も忘れずに）。",
      answer: 0,
      answerDisplay: "0, 2π/3",
      solutionSet: [0, (2 * Math.PI) / 3],
      inputAffordances: ["pi", "multi"],
      unit: "",
      unknownLabel: "√3sinθ+cosθ=1 の解（0≤θ<2π）",
      variationFromPrevious: "composite",
      compareWithStepId: "step5",
      hints: [
        {
        ...
        {
          layer: 3,
          text: "$\\sqrt{3}\\sin\\theta + \\cos\\theta = 2\\sin\\left(\\theta + \\dfrac{\\pi}{6}\\right) = 1$ より $\\sin\\left(\\theta + \\dfrac{\\pi}{6}\\right) = \\dfrac{1}{2}$。$0 \\le \\theta < 2\\pi$ のとき中身は $\\dfrac{\\pi}{6} \\le \\theta + \\dfrac{\\pi}{6} < \\dfrac{13\\pi}{6}$。この範囲で $\\sin = \\dfrac{1}{2}$ になるのは $\\dfrac{\\pi}{6}$ と $\\dfrac{5\\pi}{6}$、それぞれ $\\theta = 0$ と $\\theta = \\dfrac{2\\pi}{3}$。**合成 → 三角方程式**——$2$ つの系列が $1$ 本につながった。",
        },
      ],
      formulaPreview: "2sin(θ+π/6)=1 → 中身 = π/6, 5π/6 → θ = 0, 2π/3",
    },
    {
      id: "step10",
      position: 10,
      questionText:
        "仕上げに**変域**を付けます。$0 \\le \\theta \\le \\dfrac{\\pi}{2}$ のとき、$y = \\sin\\theta + \\cos\\theta$ の**最大値**と**最小値**を、カンマで区切って $2$ つとも答えましょう。",
      answer: Math.sqrt(2),
      answerDisplay: "√2, 1",
      solutionSet: [Math.sqrt(2), 1],
      inputAffordances: ["sqrt", "multi"],
      unit: "",
      unknownLabel: "sinθ+cosθ の最大・最小（0≤θ≤π/2）",
      variationFromPrevious: "plus_alpha",
      compareWithStepId: "step5",
      hints: [
        {
          layer: 1,
          text: "step5 と比べてみよう。合成して振幅を読むのは同じ。でも今回は $\\theta$ に**変域**が付いている——波の全部でなく一部しか使えない。step1・2 の合成結果で、中身の角はどの範囲を動く？",
        },
        {
          layer: 2,
          text: "step5 と変わったのは「変域を中身の範囲に直す」ひと手間。$y = \\sqrt{2}\\sin\\left(\\theta + \\dfrac{\\pi}{4}\\right)$ で、$0 \\le \\theta \\le \\dfrac{\\pi}{2}$ なら中身は $\\dfrac{\\pi}{4}$ から $\\dfrac{3\\pi}{4}$ まで。この範囲での $\\sin$ の最大・最小は、[単位円] の弧のどこで起きる？",
        },
        {
          layer: 3,
          text: "中身 $\\theta + \\dfrac{\\pi}{4}$ は $\\dfrac{\\pi}{4}$〜$\\dfrac{3\\pi}{4}$ を動く。この弧の上で $\\sin$（高さ）は、真上 $\\dfrac{\\pi}{2}$ で最大 $1$、両端 $\\dfrac{\\pi}{4}$・$\\dfrac{3\\pi}{4}$ で最小 $\\dfrac{1}{\\sqrt{2}}$。$\\sqrt{2}$ 倍して最大値 $\\sqrt{2}$（$\\theta = \\dfrac{\\pi}{4}$）・最小値 $1$（$\\theta = 0, \\dfrac{\\pi}{2}$）。**変域があるときは端点まで吟味**——[単位円] の弧で読むのが確実。",
        },
      ],
      formulaPreview: "中身 ∈ [π/4, 3π/4] → sin ∈ [1/√2, 1] → y ∈ [1, √2]",
    },
  ],
  derivation: `**中心の問い** ｜ ずれた $2$ つの波 $\\sin\\theta$ と $\\cos\\theta$ を足しても、ばらばらにならず、また $1$ つのきれいな波に戻るのはなぜ？——係数の組 $(a, b)$ を平面の $1$ 点と見ると、その『長さ』と『向き』が、新しい波の高さとずれそのものだとしたら？
...
これと $a\\sin\\theta + b\\cos\\theta$ を見比べると、$a = r\\cos\\alpha$・$b = r\\sin\\alpha$。この $2$ 本は「**原点から長さ $r$・向き $\\alpha$ に進むと点 $(a, b)$ に着く**」と読めます。だから：

<<TRIG_COMPOSITION_POINT>>

$$r = \\sqrt{a^2 + b^2} \\ (\\text{点までの距離・[三平方の定理]}), \\quad \\alpha = \\text{直線 } OP \\text{ の向きの角}$$

**$\\sin$ の係数 $a$ が $x$ 座標・$\\cos$ の係数 $b$ が $y$ 座標**——順番が入れ替わって見えるのは、$\\sin$ の加法定理の展開形（$\\cos\\alpha$ が $\\sin\\theta$ に付く）に合わせているから。

**ここが胚細胞**：$2$ つの波の和がまた $1$ つの波に戻るのは、**合成＝加法定理の逆読み**だから。そして波の個性（振幅 $r$・[位相] $\\alpha$）が、係数の組を**幾何**（点の長さと向き）で見るだけで手に入る。「式の問題を点の問題に翻訳する」——図形と方程式の発想が、波の世界で再演されます。

**Step 1〜4：$r$ と $\\alpha$ を別々に読む**。$\\sin\\theta + \\cos\\theta$ は $P(1,1)$ から $\\sqrt{2}\\sin\\left(\\theta+\\dfrac{\\pi}{4}\\right)$、$3\\sin\\theta + \\sqrt{3}\\cos\\theta$ は $P(3, \\sqrt{3})$ から $2\\sqrt{3}\\sin\\left(\\theta+\\dfrac{\\pi}{6}\\right)$。

**Step 5：合成の最大の使い道**。$2$ つの波のままでは山の高さが読めない。$1$ つにまとめれば **振幅がそのまま最大値**（$\\pm 2\\sqrt{3}$）。[三角関数のグラフ] の振幅の言葉がここで回収されます。

**Step 6：負の係数は象限で（質的変化）**。$-\\sin\\theta + \\cos\\theta$ の点は $P(-1, 1)$——第 $2$ 象限。$\\tan$ だけで角を決めると象限を取り違えるので、$\\cos\\alpha$・$\\sin\\alpha$ の**符号**（点の象限）まで見て $\\alpha = \\dfrac{3\\pi}{4}$。[一般角] の「座標から角を読む」の再来です。

**Step 7〜8：逆読みと、畳む先の変更**。合成形をひらくのは加法定理そのもの（Step 7）。$\\cos$ の波 $r\\cos(\\theta - \\beta)$ に畳むこともでき（Step 8）、同じ式が $2\\sin\\left(\\theta+\\dfrac{\\pi}{3}\\right) = 2\\cos\\left(\\theta-\\dfrac{\\pi}{6}\\right)$ と $2$ 通りに書ける——$\\cos$ は $\\sin$ の位相ずれだから当然、という景色。

**Step 9〜10：方程式・最大最小への合流**。$\\sqrt{3}\\sin\\theta + \\cos\\theta = 1$ は合成して $\\sin\\left(\\theta+\\dfrac{\\pi}{6}\\right) = \\dfrac{1}{2}$——[三角方程式]（系列3）に帰着。変域つきの最大最小は「**変域を中身の範囲に直して、[単位円] の弧で読む**」（Step 10）。端点の吟味を忘れないこと。

────────

**もっと深く** — 変数を $1$ つにまとめる

合成がこれほど強力なのは、$\\sin\\theta$ と $\\cos\\theta$ の **$2$ か所に散らばっていた $\\theta$ が、$1$ か所にまとまる**からです。入力 $\\theta$ から出力 $y$ までが $1$ 本の線でつながれば、方程式は逆にたどれるし、最大最小は端から端まで見渡せる。

実はこれ、二次関数の**平方完成**とまったく同じ目的の式変形です。$y = x^2 - 2x + 3$ の $x$ が $2$ か所に散らばっているのを $y = (x-1)^2 + 2$ と $1$ か所にまとめる——あの手つきの三角関数版が合成。**「変数を $1$ つにまとめる」は、関数を扱う数学全体を貫く戦略**です。

**波の重ね合わせ**：物理では、同じ振動数の $2$ つの波（音・光・電気信号）を重ねると、振幅と位相の違う $1$ つの波になる——それがまさに合成。ノイズキャンセリングは「逆位相の波を足して振幅 $0$ を作る」応用です。ベクトルの和・複素数の極形式とも同じ構造が流れています。

**出典**

- 池田洋介（2023）『数学Ⅱ・B 入門問題精講 改訂版』旺文社
  — 章構成（三角関数の合成・点 $(a, b)$ による手順・方程式と最大最小への応用・「変数を $1$ つにまとめる」という見方）を参考。問題の値はすべてオリジナル。

────────

**問いに戻ると**

「$2$ つの波を足すと、なぜまた $1$ つの波に戻る？」——合成が [加法定理] の**逆読み**だから。$r\\sin(\\theta+\\alpha)$ をひらけば必ず $a\\sin\\theta + b\\cos\\theta$ の形になる——なら逆に、その形はいつでも $1$ つの波に畳み直せる。そして畳んだ波の振幅 $r$ は点 $(a, b)$ までの**距離**、位相 $\\alpha$ は点の**向き**。係数の組を平面の $1$ 点と見た瞬間、波の個性が幾何に翻訳される。バラす（加法定理）・畳む（合成）・まとめて読む（方程式・最大最小）——三角関数の旅は、$1$ 本の定理を前からも後ろからも読めるようになって、ここで一巡りです。`,
};
