# 数学実践本『ruisuishiki数学ⅠA』実行指示書（Cursor用・2026-08-29 Fable発行／裁定反映）
...
- 各フェーズの「起動プロンプト」を新しいチャット（Agent）にコピペして使う。**1チャット=1フェーズ（または1単元）**。長い会話を引きずらない。次チャットは履歴を読まない——正はファイル（この2文書＋`~/book-math1a/`）。フェーズ末尾に短い引き継ぎブロック（成果物パス・検査結果・次に貼る起動文）を出して閉じる。
...
## 着手前裁定（2026-08-29 岩井・確定）

詳細は設計メモ §7。要約：

1. 書名『ruisuishiki数学ⅠA』（副題なし。読者＝学習者本人が主）
2. 巻末に答え一覧を置く
3. 辞書は v1 本文素文字化＋サイト案内のみ
4. 図版はサイト掲載の figureMarker すべて
5. 作業ディレクトリは `~/book-math1a/`（git init・非公開。本文は公開リポジトリに置かない）
...
起動プロンプト：
> 相談チャットの裁定は設計メモ §7 とこの指示書「着手前裁定」に書き済み。`~/book-math1a/` が無ければ mkdir（01_design/ 03_generated/ 03_integrated/ 04_audit/ 06_export/ tools/）と git init（非公開・このディレクトリのみ。公開リポジトリには置かない）。docs/book_math1a_設計メモ_2026-08-29.md と frontend/src/lib/seriesCatalog.ts を読み、『ruisuishiki数学ⅠA』に収録する部（単元）と章（系列）の一覧を、カタログの順序のまま確定してください。出力＝`~/book-math1a/01_design/章立て_v0.1.md`（書名・部見出し・系列ID・系列title・章数の集計）。数Ⅱ・数Ⅲ・大学数学・算数系列は除外。判断に迷う系列（橋渡し系列・tertiary の統計など）は「裁定待ち」欄に分けて岩井に提示。終わったら次フェーズ（P1）に貼る起動文を出して閉じる。実装（生成器）には入らない。
