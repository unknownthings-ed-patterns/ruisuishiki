...
| **3** | **完全な worked example（最終手段）**。前題の解法を示し差を**1つ**指す。**式・計算・答えを出してよい**。完全さは難度に応じてスケール（易しい同 step は簡潔／質的転換は文字式を見せる）。ヒストリー未出の式を指す「宙ぶらりん参照」を作らない | （ハードルールは Layer 1・2 のみ。Layer 3 は確実に立たせる安全網） |

公式・定義・導出は **`derivation` フィールド**（理論本文）に置く。ヒントには置かない。

比較指さしと両立する Polya 形の L2 語彙例:「これと似た、もっとかんたんな形にすると？」（公式名・手順・答えは従来どおり不可）
...
## 4. 図形と方程式ユニット（全11系列 — ✅ 完了 2026-06-26）

`python3 scripts/audit_hints.py seriesAdvanced.ts` で確認。**11系列すべて確定版お手本に準拠**。
残る△（L1違反 1〜3）はすべて数式記法による誤検出（§3 末尾の注記参照）で、実害なし。

| 系列（定数名） | 状態 |
|---|---|
| 数直線上の点 `ADV_NUMBER_LINE_SERIES` | ✅ 書き直し済み（確定版お手本） |
| 媒介変数表示 `ADV_PARAMETRIC_SERIES` | ✅ 既存お手本 |
| 直線の方程式 `ADV_LINE_EQUATION_SERIES` | ✅ 書き直し済み |
| 点と直線の距離 `ADV_POINT_LINE_DISTANCE_SERIES` | ✅ 書き直し済み |
| 円の方程式 `ADV_CIRCLE_EQUATION_SERIES` | ✅ 書き直し済み |
| 円と直線の位置関係 `ADV_CIRCLE_LINE_SERIES` | ✅ 書き直し済み |
| 円の接線 `ADV_CIRCLE_TANGENT_SERIES` | ✅ 書き直し済み |
| 束の考え方 `ADV_BUNDLE_SERIES` | ✅ 書き直し済み（step1 を Socratic 化） |
| 軌跡 `ADV_LOCUS_SERIES` | ✅ 既存お手本 |
| 領域 `ADV_REGION_SERIES` | ✅ 既存お手本 |
| 不等式と領域 `ADV_LINEAR_PROGRAMMING_SERIES` | ✅ 既存お手本 |
